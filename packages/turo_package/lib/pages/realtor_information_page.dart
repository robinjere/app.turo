    ...
    return SafeArea(
      ...
      child: Container(
        decoration: BoxDecoration(
          color:
              AppThemePreferences().appTheme.backgroundColor!.withOpacity(0.8),
          border: Border(
...
class AdditionalInfoWidget extends StatelessWidget {
    ...
    return Column(
      children: [
        if (HooksConfigurations.agentProfileConfigurationsHook(
                "additional_info") ??
            true)
          if ((HooksConfigurations.agentProfileConfigurationsHook("license") ??
                  true) &&
              (license.isNotEmpty))
            LicenseNumberWidget(license: license),
        if ((HooksConfigurations.agentProfileConfigurationsHook("tax_number") ??
                true) &&
            (taxNumber.isNotEmpty))
          TaxNumberWidget(taxNumber: taxNumber),
  ...
  bool showServicesAndSpecialities() {
    return ((HooksConfigurations.agentProfileConfigurationsHook(
                "service_areas") ??
            true) ||
        (HooksConfigurations.agentProfileConfigurationsHook("specialities") ??
            true));
  }
}
...
class MoreElevatedButtonWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return ((realtorListings.length > 2) &&
            (currentSelection == tempIndexListing))
        ? SizedBox(
            // width: 140, //160
            child: Container(
