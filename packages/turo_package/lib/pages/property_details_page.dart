...
import 'package:turo_package/pages/property_details_related_pages/report_property.dart';
...
import 'package:turo_package/widgets/custom_widgets/refresh_indicator_widget.dart';
...
import 'package:turo_package/widgets/review_related_widgets/add_review_page.dart';
...
class _PropertyDetailsPageState extends State<PropertyDetailsPage> {
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: SystemUiOverlayStyle(
        statusBarColor: AppThemePreferences()
            .appTheme
            .backgroundColor!
  ...
  getPropertyDetailPageConfigFile() async {
    var storedPropertyDetailPageConfig =
        HiveStorageManager.readPropertyDetailConfigListData();
    if (storedPropertyDetailPageConfig != null &&
        storedPropertyDetailPageConfig.isNotEmpty) {
      if (mounted) {
        setState(() {
          propertyDetailPageConfigList =
              jsonDecode(storedPropertyDetailPageConfig);
        });
      }
    } else {
      String jsonString = await rootBundle.loadString(APP_CONFIG_JSON_PATH);
      var propertyDetailPageConfig =
          propertyDetailPageLayoutFromJson(jsonString);
      if (mounted) {
        setState(() {
          propertyDetailPageConfigList =
      ...
      if (value.isNotEmpty && value[0] is Article) {
        if (mounted) {
          setState(() {
            ...
            agentDisplayOption =
                _article!.propertyInfo!.agentDisplayOption ?? "";
        ...
        if (isLoggedIn) {
          var response =
              _propertyBloc.fetchIsFavProperty(widget.propertyID.toString());
          response.then((value) {
            if (value.statusCode == 200 &&
                value.data["success"] == true &&
                value.data["is_fav"] == true) {
              if (mounted) {
                setState(() {
                  isLiked = true;
                  isInternetConnected = true;
    ...
    if (agentDisplayOption == AGENCY_INFO || agentDisplayOption == AGENT_INFO) {
      ...
      _futureAgentOrAgencyInfo!.then((dataList) {
        if (dataList.isNotEmpty) {
          ...
          tempRealtorLink =
              isAgent ? item.agentLink ?? "" : item.agencyLink ?? "";
  ...
  Future<List<dynamic>> fetchAgencyInfo(List<String> list) async {
    ...
    for (var element in tempList01) {
      List<dynamic>? tempList =
          await _propertyBloc.fetchSingleAgencyInfoList(element);
      if (tempList == null ||
          tempList.isEmpty ||
          tempList[0] == null ||
          tempList[0].runtimeType == Response) {
                ...
                Column(
                  ...
                  children: propertyDetailPageConfigList.map((map) {
                    return PropertyDetailsPageWidgets(
                      ...
                      propertyDetailsPageWidgetsListener:
                          (Map<String, dynamic> propertyDetailsPageDataMap,
                              bool isDataLoaded) {
                        if (isDataLoaded) {
      ...
      builder: (context, articleSnapshot) {
        ...
        if (articleSnapshot.hasData) {
          if (articleSnapshot.data != null &&
              articleSnapshot.data!.isNotEmpty) {
  ...
  void onFavPropertyIconPressed() async {
    if (isInternetConnected) {
      if (isLoggedIn) {
        ...
        var response = await _propertyBloc
            .fetchAddOrRemoveFromFavResponse(addOrRemoveFromFavInfo);
        ...
        if (mounted) {
          setState(() {
            if (map['added'] == true) {
              isLiked = true;
              _showToastWhileDataLoading(context,
                  UtilityMethods.getLocalizedString("add_to_fav"), false);
              GeneralNotifier()
                  .publishChange(GeneralNotifier.NEW_FAV_ADDED_REMOVED);
            } else if (map['added'] == false) {
              isLiked = false;
              _showToastWhileDataLoading(context,
                  UtilityMethods.getLocalizedString("remove_from_fav"), false);
              GeneralNotifier()
                  .publishChange(GeneralNotifier.NEW_FAV_ADDED_REMOVED);
            }
          });
        ...
        _showToastWhileDataLoading(
            ...
            UtilityMethods.getLocalizedString("you_must_login") +
                UtilityMethods.getLocalizedString("before_adding_to_favorites"),
            true);
      }
    }
  }
  ...
  Widget bottomActionBarWidget() {
    return PropertyProfileBottomButtonActionBar(
      ...
      article: _article,
      realtorInfoMap: _realtorInfoMap,
      profileBottomButtonActionBarListener:
          (bool isBottomButtonActionBarDisplayed) {
  ...
  Widget noResultFoundPage() {
    return NoResultErrorWidget(
      ...
      bodyErrorText: UtilityMethods.getLocalizedString(
          "no_properties_error_message_search_by_id"),
  ...
  void navigateToAddReviewPage() {
    isLoggedIn
        ...
        : _showToastWhileDataLoading(
            context,
            UtilityMethods.getLocalizedString("you_must_login") +
                UtilityMethods.getLocalizedString("before_leaving_a_review"),
            true,
          );
  }
          ...
          onPressed: () async {
            ...
            isLoggedIn
                ? Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => ReportProperty(
                        informationMap: {
                          ...
                          SEND_EMAIL_MESSAGE: UtilityMethods.getLocalizedString(
                              ...
                              inputWords: [
                                UtilityMethods.stripHtmlIfNeeded(
                          ...
                          SEND_EMAIL_UNIQUE_ID:
                              widget.article!.propertyInfo!.uniqueId!,
                          SEND_EMAIL_SOURCE: PROPERTY,
                          SEND_EMAIL_LISTING_LINK: _article!.link!.isNotEmpty
                              ? _article!.link!
                              : widget.article!.link,
                        },
                      ),
                ...
                : _showToastWhileDataLoading(
                    context,
                    UtilityMethods.getLocalizedString(
                        "You must log in before reporting a property"),
                    true);
          },
          child: GenericTextWidget(
    ...
    return Positioned(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border(
            bottom: BorderSide(
              width: AppThemePreferences
                  .propertyDetailsPageTopBarDividerBorderWidth,
              color: AppThemePreferences()
                  .appTheme
                  .propertyDetailsPageTopBarDividerColor!
                  .withOpacity(opacity),
            ),
          ),
          boxShadow: [
            BoxShadow(
              color: AppThemePreferences()
                  .appTheme
                  .propertyDetailsPageTopBarShadowColor!
                  .withOpacity(opacity),
              ...
              children: [
                    ...
                    child: Container(
                      ...
                      child: GenericTextWidget(
                        ...
                        style: AppThemePreferences()
                            .appTheme
                            .propertyDetailsPageTopBarTitleTextStyle,
                      ),
                    ),
                  ),
                ),
                ...
                Expanded(
                  ...
                  child: TopBarButtonWidget(
                    ...
                    iconColor:
                        isLiked ? AppThemePreferences.favouriteIconColor : null,
  ...
  Widget build(BuildContext context) {
    return CircleAvatar(
      ...
      radius: circularRadius ??
          AppThemePreferences.propertyDetailsPageTopBarCircularAvatarRadius,
      child: IconButton(
        icon: Icon(
          iconData,
          color: iconColor ??
          ...
          size: iconSize ??
              AppThemePreferences.propertyDetailsPageTopBarIconsIconSize,
        ),
        onPressed: onPressed,
      ),
    );
