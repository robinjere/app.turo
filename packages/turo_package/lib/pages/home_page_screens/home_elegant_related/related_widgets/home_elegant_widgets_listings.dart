...
import 'package:turo_package/models/terms_with_icon.dart';
...
import 'package:turo_package/widgets/blogs_related/blogs_listing_widget.dart';
import 'package:turo_package/widgets/dynamic_widgets/terms_with_icons_widget.dart';
import 'package:turo_package/widgets/generic_text_widget.dart';
import 'package:turo_package/widgets/header_widget.dart';
import 'package:turo_package/pages/home_page_screens/parent_home_related/home_screen_widgets/home_screen_properties_related_widgets/explore_properties_widget.dart';
...
import 'package:turo_package/pages/home_page_screens/parent_home_related/home_screen_widgets/home_screen_recent_searches_widget/home_screen_recent_searches_widget.dart';
import 'package:turo_package/widgets/partners_widget/partner_widget.dart';
import 'package:turo_package/widgets/type_status_row_widget.dart';
...
class _HomeElegantListingsWidgetState extends State<HomeElegantListingsWidget> {
          ...
          if (homeConfigMap[subTypeValueCityKey] != cityId) {
            setState(() {
              ...
              homeConfigMap[titleKey] =
                  titleForSectionBasedOnCitySelection(city);
            });

            loadData();
          }
        } else if (homeConfigMap[sectionTypeKey] == propertyKey &&
            listDependsOnCitySelection()) {
          setState(() {
            Map<String, dynamic> map =
                HiveStorageManager.readSelectedCityInfo() ?? {};
            String cityId = UtilityMethods.valueForKeyOrEmpty(map, CITY_ID);
            if (homeConfigMap[subTypeValueCityKey] != cityId) {
              homeScreenList = [];
              isDataLoaded = false;
  ...
  Future<List<dynamic>> fetchRelatedList(BuildContext context, int page) async {
    ...
    try {
      ...
      /// Fetch latest and city selected properties (old)
        ...
        String city = UtilityMethods.valueForKeyOrEmpty(map, CITY);
        String cityId = UtilityMethods.valueForKeyOrEmpty(map, CITY_ID);
        homeConfigMap[titleKey] = titleForSectionBasedOnCitySelection(city);
        if (city.isNotEmpty) {
      ...
      /// Fetch Blogs list
        ...
        var (success, internet, blogsData) = await _propertyBloc
            .fetchBlogArticles({"page": "1", "per_page": "20"});
  ...
  setRouteToNavigate() async {
    ...
    if (homeConfigMap[sectionTypeKey] == featuredPropertyKey) {
      ...
      Map<String, dynamic> dataMap = {};
      if (listDependsOnCitySelection()) {
        Map<String, dynamic> cityInfoMap =
            HiveStorageManager.readSelectedCityInfo() ?? {};
        String citySlug =
            UtilityMethods.valueForKeyOrEmpty(cityInfoMap, CITY_SLUG);
      ...
      route = (context) => AllBlogsPage(
          ...
          blogDesign: UtilityMethods.getDesignValue(homeConfigMap[designKey]) ??
              DESIGN_01);
    } else if (homeConfigMap[sectionTypeKey] == propertyKey) {
      Map<String, dynamic> dataMap = {};
      ...
      bool featured = dataMap[showFeaturedKey] != null &&
              dataMap[showFeaturedKey] is bool &&
              dataMap[showFeaturedKey]
          ? true
          : false;
  ...
  Widget build(BuildContext context) {
    ...
    if (homeConfigMap[sectionTypeKey] == blogsKey &&
      ...
      BLOGS_DESIGN =
          UtilityMethods.getDesignValue(homeConfigMap[designKey]) ?? DESIGN_01;
    }
                ...
                if (homeConfigMap[sectionTypeKey] == allPropertyKey ||
                  ...
                  if (isDataLoaded)
                    PropertiesListingGenericWidget(
                      ...
                      design: UtilityMethods.getHomePropertyItemDesignName(
                          homeConfigMap),
                      listingView: homeConfigMap[sectionListingViewKey] ??
                          homeScreenWidgetsListingCarouselView,
                    )
  ...
  bool listDependsOnCitySelection() {
    List subTypeList =
        UtilityMethods.listForKeyOrEmpty(homeConfigMap, subTypeListKey);
    List subTypeValueList =
        UtilityMethods.listForKeyOrEmpty(homeConfigMap, subTypeValueListKey);
  ...
  String titleForSectionBasedOnCitySelection(String city) {
    String title = "";
    ...
    if (city.isNotEmpty && city.toLowerCase() != 'all') {
      if (titleKeyOriginal.contains("{city_name}")) {
        ...
        title = UtilityMethods.getLocalizedString("latest_properties_in_city",
            inputWords: [city]);
      }
    }
