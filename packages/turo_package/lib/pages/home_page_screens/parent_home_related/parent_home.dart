...
import 'package:turo_package/pages/home_page_screens/parent_home_related/home_screen_widgets/home_screen_listing_widgets/generic_home_screen_listings.dart';
...
class ParentHomeState<T extends ParentHome> extends State<T> {
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: HomeScreenUtilities()
          .getSystemUiOverlayStyle(design: HOME_SCREEN_DESIGN),
      child: SafeArea(
        child: Scaffold(
          ...
          setState(() {
            ...
            if (success && checkNotifications != null) {
              receivedNewNotifications =
                  checkNotifications.hasNotification ?? false;
            }
          });
        }
        return null;
    ...
    if (mounted) {
      setState(() {
        ...
        if (cityData.isNotEmpty) {
          String oldSelectedCityId =
              UtilityMethods.valueForKeyOrEmpty(cityData, CITY_ID);
          if (oldSelectedCityId != _selectedCityId) {
            saveSelectedCityInfo(
                _selectedCityId, _selectedCity, _selectedCitySlug);
          } else {
            // Do not update
          }
        } else {
