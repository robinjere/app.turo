...
class RealtorListingsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double height = tag == AGENTS_TAG
        ? AppThemePreferences.agentsListContainerHeight
        : AppThemePreferences.agenciesListContainerHeight;
    double width = tag == AGENTS_TAG
        ? AppThemePreferences.agentsListContainerWidth
        : AppThemePreferences.agenciesListContainerWidth;

    return Container(
      // padding: listingView == homeScreenWidgetsListingListView ? null :  EdgeInsets.only(top: 5, left: 20),
      padding: listingView == homeScreenWidgetsListingListView
      ...
      child: ListView.builder(
        scrollDirection: listingView == homeScreenWidgetsListingListView
            ? Axis.vertical
            : Axis.horizontal,
        ...
        itemBuilder: (context, index) {
          ...
          return Container(
            ...
            padding: EdgeInsets.only(
                bottom: 10,
                right: listingView == homeScreenWidgetsListingListView ? 0 : 5),
            child: CardWidget(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (tag == AGENTS_TAG && item is Agent)
                                RealtorPositionWidget(
                                    position: "${item.agentPosition}"),
                              if (tag == AGENTS_TAG && item is Agent)
                                RealtorNameWidget(
                                    title: "${item.agentCompany}", tag: tag),
