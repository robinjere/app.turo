...
import 'package:turo_package/pages/crm_pages/crm_bottom_bar.dart';
import 'package:turo_package/pages/home_screen_drawer_menu_pages/add_property_request.dart';
...
import 'package:turo_package/pages/home_page_screens/parent_home_related/home_screen_widgets/home_screen_drawer_widgets/home_screen_drawer_widget/home_screen_drawer_list_tile.dart';
import 'package:turo_package/pages/home_screen_drawer_menu_pages/all_agency.dart';
import 'package:turo_package/pages/home_screen_drawer_menu_pages/all_agents.dart';
import 'package:turo_package/pages/home_screen_drawer_menu_pages/all_blogs.dart';
import 'package:turo_package/pages/home_screen_drawer_menu_pages/favorites.dart';
import 'package:turo_package/pages/home_screen_drawer_menu_pages/my_agency_agents.dart';
...
import 'package:turo_package/pages/home_screen_drawer_menu_pages/request_demo.dart';
...
import 'package:turo_package/pages/main_screen_pages/my_home_page.dart';
...
class HomeScreenDrawerWidgetsListing extends StatefulWidget {
  ...
  final HomeScreenDrawerWidgetsListingListener
  ...
  @override
  State<HomeScreenDrawerWidgetsListing> createState() =>
      _HomeScreenDrawerWidgetsListingState();
}

class _HomeScreenDrawerWidgetsListingState
    extends State<HomeScreenDrawerWidgetsListing> {
  ...
  void initState() {
    ...
    if (drawerConfigItemMap[sectionTypeKey] == expansionTileSectionType) {
      expansionTileChildren =
          drawerConfigItemMap[expansionTileChildrenSectionType] ?? [];
    }
  ...
  Widget build(BuildContext context) {
    if (isMenuDrawerHelper && menuDrawerHelper != null) {
      if (menuDrawerHelper!.enable!) {
        if (menuDrawerHelper!.expansionTileChildren.isNotEmpty) {
          ...
          return HomeScreenDrawerListTile(
            selectedHome: _selectedHome,
            title: UtilityMethods.getLocalizedString(
                menuDrawerHelper!.title ?? ""),
    ...
    return drawerConfigItemMap.isNotEmpty &&
            (drawerConfigItemMap[enableKey] ?? false)
        ? Column(
            children: [
              if (drawerConfigItemMap[sectionTypeKey] ==
                  PLACE_HOLDER_SECTION_TYPE)
                HooksConfigurations.drawerWidgetsHook(
                        context, drawerConfigItemMap[titleKey]) ??
                    Container()
              else if (drawerConfigItemMap[sectionTypeKey] !=
                  expansionTileSectionType)
                getListTile(
  ...
  Widget getListTile({
    ...
    if (itemSectionType == logoutSectionType && isUserLogged) {
      ...
      return HomeScreenDrawerListTile(
          ...
          iconData: getSectionIcon(context, itemSectionType),
          sectionType: itemSectionType,
          builder: itemMap["on_tap"] ??
              getWidgetBuilder(context, itemSectionType, itemMap[dataMapKey],
                  menuItem: itemMap),
      ...
      if (itemSectionType == loginSectionType && isUserLogged) {
        ...
        if (itemSectionType == myAgentsSectionType && !isUserLogged) {
          return Container();
        } else if (itemSectionType == myAgentsSectionType &&
            userInfoMap[USER_ROLE] != USER_ROLE_HOUZEZ_AGENCY_VALUE) {
          return Container();
        }
  ...
  bool isHomeSection(String sectionType) {
    if (sectionType == homeSectionType ||
        sectionType == home0SectionType ||
        sectionType == home01SectionType ||
        sectionType == home02SectionType ||
  ...
  WidgetBuilder? getWidgetBuilder(
    ...
    if (sectionType == quickAddPropertySectionType)
      return (context) => UtilityMethods.navigateToAddPropertyPage(
          navigateToQuickAdd: true); //QuickAddPropertyV2();
    if (sectionType == addPropertySectionType)
      return (context) => UtilityMethods.navigateToAddPropertyPage();
    ...
    if (sectionType == myAgentsSectionType)
      return (context) => MyAgencyAgents();
    if (sectionType == requestPropertySectionType)
      return (context) => AddPropertyRequest();
    ...
    if (sectionType == savedSearchesSectionType)
      return (context) => SavedSearches(showAppBar: true);
    if (sectionType == activitiesSectionType)
      return (context) => ActivitiesFromBoard();
    if (sectionType == inquiriesSectionType)
      return (context) => InquiriesFromBoard();
    ...
    if (sectionType == settingsSectionType)
      return (context) => HomePageSettings();
    if (sectionType == requestDemoSectionType)
      return (context) => ContactDeveloper();
    ...
    if (sectionType == menuTermSectionType) {
      ...
      if (dataMap != null) {
        ...
        if (dataMap.containsKey(SEARCH_SUB_LIST_TYPE_KEY) &&
          ...
          searchSubListTypeList =
              List<String>.from(dataMap[SEARCH_SUB_LIST_TYPE_KEY]);
        }
    ...
    if (sectionType == aboutScreenSectionType) {
      return (context) => SHOW_DEMO_CONFIGURATIONS
          ? About()
    ...
    if (sectionType == privacyPolicyScreenSectionType) {
      return (context) => WebPage(
          APP_PRIVACY_URL, UtilityMethods.getLocalizedString("privacy_policy"));
    }
    if (sectionType == termsAndConditionsScreenSectionType) {
      return (context) => WebPage(APP_TERMS_URL,
    ...
    if (sectionType == webUrlSectionType) {
      ...
      return (context) => WebPage(menuItem[dataMapKey][webUrlDataMapKey],
          UtilityMethods.getLocalizedString(menuItem[titleKey]));
    }
    ...
    if (sectionType == blogsKey) {
      return (context) =>
          AllBlogsPage(title: menuItem[titleKey], blogDesign: BLOGS_DESIGN);
    }
