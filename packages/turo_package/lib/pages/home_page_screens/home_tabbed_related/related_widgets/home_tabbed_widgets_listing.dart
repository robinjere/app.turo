...
typedef HomeTabbedListingsWidgetListener = void Function(
    bool errorWhileLoading, bool dataLoadingComplete);
...
class HomeTabbedListingsWidget extends StatefulWidget {
  ...
  @override
  State<HomeTabbedListingsWidget> createState() =>
      _HomeScreenListingsWidgetState();
}

class _HomeScreenListingsWidgetState extends State<HomeTabbedListingsWidget>
    with AutomaticKeepAliveClientMixin<HomeTabbedListingsWidget> {
  int page = 1;
      ...
      if (GeneralNotifier().change == GeneralNotifier.CITY_DATA_UPDATE) {
        if (homeConfigMap[sectionTypeKey] == allPropertyKey &&
          ...
          setState(() {
            ...
            if (homeConfigMap[subTypeValueKey] != cityId) {
              homeScreenList = [];
              isDataLoaded = false;
    ...
    try {
      ...
      /// Fetch Properties
        ...
        if (homeConfigMap[subTypeKey] == propertyCityDataType &&
          ...
          if (map.isNotEmpty && cityId.isNotEmpty) {
            String city = UtilityMethods.valueForKeyOrEmpty(map, CITY);
            if (homeConfigMap[titleKey] != "Please Select") {
              homeConfigMap[titleKey] = "";
              homeConfigMap[titleKey] = UtilityMethods.getLocalizedString(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.refresh &&
        ...
        homeConfigMap[sectionTypeKey] != recentSearchKey &&
        homeConfigMap[sectionTypeKey] != PLACE_HOLDER_SECTION_TYPE) {
      if (isPageFreeForLoading &&
          mapEquals(widget.selectedItem, widget.homeScreenData)) {
        homeScreenList = [];
        isDataLoaded = false;
        noDataReceived = false;
                ...
                if (homeConfigMap[sectionTypeKey] == termWithIconsTermKey)
                  Container(
                    ...
                    child: _termsWithIconList.isNotEmpty
                        ? DynamicTermsWithIconWidget(
                            dataList: _termsWithIconList)
                        : TermWithIconsWidget(),
                  ),
                if (homeConfigMap[sectionTypeKey] == allPropertyKey ||
  ...
  setRouteToNavigate() async {
    ...
    if (homeConfigMap[sectionTypeKey] == featuredPropertyKey) {
      ...
      route = (context) => AllBlogsPage(
            ...
            blogDesign:
                UtilityMethods.getDesignValue(homeConfigMap[designKey]) ??
                    DESIGN_01,
          );
    } else {
      route = null;
