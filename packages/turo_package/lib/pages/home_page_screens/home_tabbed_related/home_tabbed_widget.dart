...
import 'package:turo_package/pages/home_page_screens/home_tabbed_related/related_widgets/home_tabbed_sliver_app_bar.dart';
import 'package:turo_package/pages/home_page_screens/home_tabbed_related/related_widgets/home_tabbed_widgets_listing.dart';
...
class HomeTabbed extends ParentSliverHome {
  final GlobalKey<ScaffoldState>? scaffoldKey;

  const HomeTabbed({Key? key, this.scaffoldKey}) : super(key: key);

  @override
  _HomeTabbedState createState() => _HomeTabbedState();
}

class _HomeTabbedState extends ParentSliverHomeState<HomeTabbed>
  ...
  getHomeConfigFile() {
    if (mounted) {
      setState(() {
        ...
        homeConfigList
            .removeWhere((homeItem) => (homeItem[sectionTypeKey] == adKey));

        // Set the initial index of TabBar
        for (var item in homeConfigList) {
          if (item[sectionTypeKey] != termWithIconsTermKey &&
              item[sectionTypeKey] != recentSearchKey) {
        ...
        NestedScrollView(
          headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
            return <Widget>[
              ...
              HomeTabbedSliverAppBar(
                ...
                homeTabbedSliverAppBarListener: (
                    {filterInfo, hideNotificationDot}) {
                  if (filterInfo != null && filterInfo.isNotEmpty) {
                    updateData(filterInfo);
                  }
          ...
          body: TabBarView(
            ...
            children: homeConfigList.map((item) {
              return item[sectionTypeKey] == recentSearchKey
                  ? SingleChildScrollView(
                      child: HomeTabbedListingsWidget(
                        homeScreenData: item,
                        selectedItem: selectedHomeConfigItem,
                        homeTabbedListingsWidgetListener:
                            (bool errorOccur, bool dataLoadingComplete) {},
                      ),
                    )
                  : RefreshIndicatorWidget(
                      edgeOffset: 0.0,
                      onRefresh: () async {
                        if (!isRefreshStateBusy &&
                            mapEquals(selectedHomeConfigItem, item)) {
