...
import 'package:turo_package/files/item_design_files/item_design_notifier.dart';
import 'package:turo_package/models/article.dart';
import 'package:turo_package/models/property_meta_data.dart';
import 'package:turo_package/widgets/no_internet_error_widget.dart';
import 'package:turo_package/widgets/search_result_widgets/map_prop_list_widget.dart';
import 'package:turo_package/widgets/search_result_widgets/search_results_builder_widget.dart';
import 'package:turo_package/widgets/search_result_widgets/search_bar_widget.dart';
import 'package:turo_package/widgets/search_result_widgets/sliding_panel_widget.dart';
import 'package:turo_package/widgets/search_result_widgets/top_container_widget.dart';
import 'package:provider/provider.dart';
import 'package:sliding_up_panel/sliding_up_panel.dart';
import 'package:turo_package/files/theme_service_files/theme_storage_manager.dart';
...
typedef SearchPageListener = void Function(
    Map<String, dynamic> filterDataMap, String closeOption);
...
class SearchResultState extends State<SearchResult> {
  ...
  Map<String, dynamic> chipsSearchDataMap =
      {}; // Used this map to navigate to Filter Screen
  ...
  void initState() {
    ...
    if (widget.searchRelatedData != null &&
        widget.searchRelatedData!.isNotEmpty) {
      if (widget.searchRelatedData!.containsKey(REALTOR_SEARCH_TYPE)) {
        if (mounted) {
          setState(() {
            if (widget.searchRelatedData![REALTOR_SEARCH_TYPE] ==
                REALTOR_SEARCH_TYPE_AUTHOR) {
              isAuthor = true;
            }
  ...
  Widget build(BuildContext context) {
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: WillPopScope(
        onWillPop: () {
          widget.searchPageListener!(
              HiveStorageManager.readFilterDataInfo(), CLOSE);
              ...
              body: hasInternet == false
                  ? Center(
                      child: NoInternetConnectionErrorWidget(
                          onPressed: () => checkInternetAndSearch()))
                  : Stack(
                        ...
                        SlidingPanelWidget(
                              ...
                              setState(() {
                                ...
                                if (selectedMarkerPropertyId != null) {
                                  if (selectedMarkerPropertyId != -1) {
                                    _filteredArticlesList.removeWhere(
                                        (element) => element is AdWidget);
                                    int index = _filteredArticlesList
                                        .indexWhere((element) {
                                    ...
                                    if (index != -1 &&
                                      ...
                                      int currentPage =
                                          carouselPageController!.page!.toInt();
                          ...
                          panelBuilder: (panelScrollController) {
                            panelScrollController.addListener(() {
                              if (panelScrollController.position.pixels ==
                                  ...
                                  if (!_infiniteStop && _isPaginationFree) {
                                    ...
                                    setState(() {
                                      ...
                                      _futureFilteredArticles =
                                          fetchFilteredArticles(page, perPage,
                                              filteredMapForSearch,
                                              fetchSubListing:
                                                  widget.fetchSubListing);
                                    });
                                    // print("Page: $page........");
                                    _isAtBottom = false;
                                  }
  ...
  void doSearch() {
    ...
    if (isAuthor) {
      filterChipsRelatedList = [
        ...
        {
          REALTOR_CHIP_KEY:
              "${UtilityMethods.getLocalizedString(widget.searchRelatedData![REALTOR_SEARCH_TYPE])} : $realtorName",
        },
      ];
    } else {
      if (mapFromFilterScreen.isNotEmpty) {
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, BEDROOMS)) {
          ...
          List<String> tempBedroomStringList =
              List<String>.from(mapFromFilterScreen[BEDROOMS]);
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, BATHROOMS)) {
          ...
          List<String> tempBathroomStringList =
              List<String>.from(mapFromFilterScreen[BATHROOMS]);
          if (tempBathroomStringList.isNotEmpty) {
            ...
            tempBathroomStringList =
                UtilityMethods.getSanitizedStringPickerList(
                    tempBathroomStringList);
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_STATUS_SLUG)) {
          var status =
              List<String>.from(mapFromFilterScreen[PROPERTY_STATUS_SLUG]);
          var nonEmpty = status
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_STATUS)) {
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_TYPE)) {
          ...
          tempPropertyTypeSlugsList =
              mapFromFilterScreen[PROPERTY_TYPE_SLUG] ?? [];
          if (tempPropertyTypeSlugsList.isNotEmpty &&
              mapFromFilterScreen[termPickerTypeKey] != boxTermPicker) {
            String itemSlug = tempPropertyTypeSlugsList[0];
            Term? obj = UtilityMethods.getPropertyMetaDataObjectWithSlug(
                dataType: PROPERTY_TYPE, slug: itemSlug);
            if (obj != null && obj.parent != 0) {
              Term? parentObj = UtilityMethods.getPropertyMetaDataObjectWithId(
                  dataType: PROPERTY_TYPE, id: obj.parent!);
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_LABEL)) {
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_FEATURES)) {
        ...
        if (UtilityMethods.isMapItemPopulated(
          ...
          var type = propertyTypeSlug is String
              ? [propertyTypeSlug]
              : List<String>.from(propertyTypeSlug);
          var nonEmpty = type
              .where((element) => element.isNotEmpty && element != "all")
              .toList();
          ...
          if (nonEmpty.isNotEmpty && nonEmpty.length > 1) {
            ...
            Map<String, dynamic>? propertyDataMap =
                HiveStorageManager.readPropertyTypesMapData();
            if (propertyDataMap != null && propertyDataMap.isNotEmpty) {
              ...
              for (var item in propertyTypeList) {
                if (keys.contains(item)) {
                  Term? obj =
                      UtilityMethods.getPropertyMetaDataObjectWithItemName(
                          dataType: PROPERTY_TYPE, name: item);
                  if (obj != null) {
        ...
        if (UtilityMethods.isMapItemPopulated(
          ...
          var label =
              List<String>.from(mapFromFilterScreen[PROPERTY_LABEL_SLUG]);
          var nonEmpty = label
              .where((element) => element.isNotEmpty && element != "all")
              .toList();

          filteredMapForSearch[SEARCH_RESULTS_LABEL] = nonEmpty;
          chipsSearchDataMap[PROPERTY_LABEL_SLUG] =
              mapFromFilterScreen[PROPERTY_LABEL_SLUG];
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, CITY_SLUG)) {
          ...
          var nonEmpty =
              tempSlugList.where((element) => element != "all").toList();
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_AREA_SLUG)) {
          var areas =
              List<String>.from(mapFromFilterScreen[PROPERTY_AREA_SLUG]);
          var nonEmpty = areas
              .where((element) => element.isNotEmpty && element != "all")
              .toList();
          if (nonEmpty.isNotEmpty) {
        ...
        if (UtilityMethods.isMapItemPopulated(
          ...
          filteredMapForSearch[SEARCH_RESULTS_KEYWORD] =
              mapFromFilterScreen[PROPERTY_KEYWORD];
          filterChipsRelatedList.add({
            PROPERTY_KEYWORD: mapFromFilterScreen[PROPERTY_KEYWORD],
          });

          chipsSearchDataMap[PROPERTY_KEYWORD] =
              mapFromFilterScreen[PROPERTY_KEYWORD];
        }

        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_COUNTRY_SLUG)) {
          var countries =
              List<String>.from(mapFromFilterScreen[PROPERTY_COUNTRY_SLUG]);
          var nonEmpty = countries
              .where((element) => element.isNotEmpty && element != "all")
              .toList();
          if (nonEmpty.isNotEmpty) {
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_STATE_SLUG)) {
          var states =
              List<String>.from(mapFromFilterScreen[PROPERTY_STATE_SLUG]);
          var nonEmpty = states
              .where((element) => element.isNotEmpty && element != "all")
              .toList();
          if (nonEmpty.isNotEmpty) {
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_FEATURES_SLUG)) {
          var features =
              List<String>.from(mapFromFilterScreen[PROPERTY_FEATURES_SLUG]);
          var nonEmpty = features
              .where((element) => element.isNotEmpty && element != "all")
              .toList();
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, AREA_MAX)) {
          filteredMapForSearch[SEARCH_RESULTS_MAX_AREA] =
              mapFromFilterScreen[AREA_MAX];
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, USE_RADIUS)) {
          chipsSearchDataMap[USE_RADIUS] = mapFromFilterScreen[USE_RADIUS];
          chipsSearchDataMap[SEARCH_LOCATION] =
              mapFromFilterScreen[SEARCH_LOCATION];
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_CUSTOM_FIELDS)) {
          Map tempMap = mapFromFilterScreen[PROPERTY_CUSTOM_FIELDS];

          chipsSearchDataMap[PROPERTY_CUSTOM_FIELDS] =
              mapFromFilterScreen[PROPERTY_CUSTOM_FIELDS];
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, AREA_MIN) &&
          ...
          filterChipsRelatedList.add({
            AREA_MAX:
                "${mapFromFilterScreen[AREA_MIN]} - ${mapFromFilterScreen[AREA_MAX]}",
          });
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PRICE_MIN) &&
          ...
          filterChipsRelatedList.add({
            PRICE_MAX:
                "${mapFromFilterScreen[PRICE_MIN]} - ${mapFromFilterScreen[PRICE_MAX]}",
          });
        }

        if (UtilityMethods.getBooleanItemValueFromMap(
            inputMap: mapFromFilterScreen, key: showFeaturedKey)) {
          filteredMapForSearch[SEARCH_RESULTS_FEATURED] = "1";
          filterChipsRelatedList.add({
            FEATURED_CHIP_KEY:
                UtilityMethods.getLocalizedString(FEATURED_CHIP_VALUE),
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_AREA)) {
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_STATE)) {
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_COUNTRY)) {
        ...
        if (mapFromFilterScreen.containsKey(REALTOR_SEARCH_TYPE)) {
          filterChipsRelatedList.add({
            REALTOR_CHIP_KEY:
                "${UtilityMethods.getLocalizedString(mapFromFilterScreen[REALTOR_SEARCH_TYPE])} : ${mapFromFilterScreen[REALTOR_SEARCH_NAME]}",
          });

          if (mapFromFilterScreen[REALTOR_SEARCH_TYPE] ==
              REALTOR_SEARCH_TYPE_AGENT) {
            filteredMapForSearch[REALTOR_SEARCH_AGENT] =
                mapFromFilterScreen[REALTOR_SEARCH_ID];
          } else if (mapFromFilterScreen[REALTOR_SEARCH_TYPE] ==
              REALTOR_SEARCH_TYPE_AGENCY) {
            filteredMapForSearch[REALTOR_SEARCH_AGENCY] =
                mapFromFilterScreen[REALTOR_SEARCH_ID];
          }
        }
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, metaKeyFiltersKey)) {
        ...
        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, keywordFiltersKey)) {
  ...
  Future<List<dynamic>> fetchFilteredArticles(
    ...
    if (isAuthor || fetchSubListing) {
      ...
      if (isAuthor) {
        tempList = await _propertyBloc.fetchAllProperties(
            'any', page, perPage, realtorId);
      } else if (fetchSubListing) {
        tempList =
            await _propertyBloc.fetchMultipleArticles(widget.subListingIds);
      }
    ...
    Map<String, dynamic> tempFilterDataMap =
