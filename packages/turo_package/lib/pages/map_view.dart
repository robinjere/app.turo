...
import 'package:turo_package/models/map_marker.dart';
...
import 'package:turo_package/widgets/search_result_widgets/label_marker.dart';
import 'dart:convert';
import 'dart:math';
...
class _MapViewState extends State<MapView>
  ...
  EdgeInsets mapPaddingActive =
      EdgeInsets.only(left: 5, right: 5, top: 150, bottom: 200);
  ...
  MapMarker findMarkerById(String markerId) {
    return markers.firstWhere(
      ...
      orElse: () =>
          MapMarker(id: '', position: LatLng(0, 0)), // Default if not found
    );
  }
  ...
  updateGoogleMapMarkers() async {
    ...
    for (MapMarker item in _tempMapMarkersList) {
      ...
      if (_selectedMarker != null &&
          _selectedMarker!.position == item.position) {
    ...
    if (_selectedHeroId != null && _selectedMarker != null) {
      if (!addedSelectedMarker) {
        ...
        print(
            "animateCameraToSelectedProperty():: got cluster with count: ${_tempMapMarkersList.length}");
      ...
      _googleMapController?.isMarkerInfoWindowShown(markerId).then((shown) {
        if (!shown) {
          _googleMapController!
              .showMarkerInfoWindow(MarkerId(_selectedHeroId!));
        }
      });
    }
    // Release the memory:
  ...
  animateCameraToSelectedProperty() async {
    ...
    if (widget.snapCameraToSelectedIndex &&
      ...
      if (mapMarkerCache.containsKey(item.id)) {
        ...
        if (_selectedMarker != null &&
            _selectedMarker!.position == marker.position) {
          return;
        }

        _selectedMarker = marker;
        print(
            "animateCameraToSelectedProperty():: added new marker: ${_selectedMarker}");
                  ...
                  : GoogleMap(
                      ...
                      padding: _googleMapController == null
                          ...
                          : const EdgeInsets.only(
                              left: 20, right: 20, top: 140, bottom: 250),
  ...
  LatLngBounds generateBoundingBox(LatLng center) {
    ...
    double lonOffset = bufferDistance /
        (earthRadius * cos(pi * center.latitude / 180.0)) *
        (180.0 / pi);
    ...
    return LatLngBounds(
        northeast: LatLng(neLat, neLon), southwest: LatLng(swLat, swLon));
  }
}
