...
import 'package:turo_package/pages/crm_pages/crm_pages_widgets/board_pages_widgets.dart';
import 'package:turo_package/pages/crm_pages/crm_webservices_manager/crm_repository.dart';
...
import 'package:turo_package/widgets/generic_text_field_widgets/drop_down_widget.dart';
...
typedef DealDetailPageListener = void Function(
    {int index, bool refresh, String dealOption});
...
class _DealDetailPageState extends State<DealDetailPage> {
  ...
  final TextEditingController _actionDueTextController =
      TextEditingController();
  final TextEditingController _lastContactTextController =
      TextEditingController();
  ...
  loadData() {
    ...
    String? dealNextAction =
        HiveStorageManager.readNDealNextActionInfoData() ?? "";
    ...
    if (widget.deal.actionDueDate! != "0000-00-00 00:00:00") {
      actionDueSelectedDate =
          DateFormat("yyyy-MM-dd").parse(widget.deal.actionDueDate!);
    ...
    if (widget.deal.lastContactDate! != "0000-00-00 00:00:00") {
      lastContactSelectedDate =
          DateFormat("yyyy-MM-dd").parse(widget.deal.lastContactDate!);
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            CRMIconAndText(AppThemePreferences.agentsIcon,
                                widget.deal.agentName),
                            CRMIconAndText(AppThemePreferences.priceTagIcon,
                                widget.deal.dealValue),
                          ],
                        ),
                        const CRMDetailGap(gap: 20),
                        CRMIconAndText(AppThemePreferences.showCharts, "status",
                            addBottomPadding: true),
                        dropDownWidget(
                          "status",
                          statusOptionsList,
                        ...
                        const CRMDetailGap(gap: 20),
                        CRMIconAndText(AppThemePreferences.timerOutlined,
                            "action_due_date",
                            addBottomPadding: true),
                        datePickerWidget(DEAL_SET_ACTION_DUE_DATE),
                        const CRMDetailGap(gap: 20),
                        CRMIconAndText(AppThemePreferences.filterCenterFocus,
                            "next_action",
                            addBottomPadding: true),
                        dropDownWidget(
                          "next_action",
                          nextActionOptionsList,
                          _nextActionValue,
                          DEAL_SET_NEXT_ACTION,
                        ),
                        const CRMDetailGap(gap: 20),
                        CRMIconAndText(
                            AppThemePreferences.calendarMonthOutlined,
                            "last_contact_date",
                            addBottomPadding: true),
  ...
  Widget datePickerWidget(String key) {
    return TextFormFieldWidget(
      ...
      controller: key == DEAL_SET_ACTION_DUE_DATE
          ? _actionDueTextController
          : _lastContactTextController,
      readOnly: true,
    );
  }
  ...
  Future<void> _selectDateActionDue(BuildContext context, String key) async {
    ...
    if (picked != null && picked != actionDueSelectedDate) {
      ...
      widget.dealDetailPageListener!(
          refresh: true, dealOption: widget.deal.dealGroup!);
    }
  }
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => AddNewDeal(
                    dealDetailMap: widget.dealDetailMap,
                    forEditDeal: true,
                    addNewDealPageListenerPageListener:
                        (bool refresh, String dealOption) {
                      widget.dealDetailPageListener!(
                          refresh: true, dealOption: dealOption);
              ...
              actions: <Widget>[
                ...
                TextButtonWidget(
                  onPressed: () async {
                    ...
                    final response =
                        await _crmRepository.fetchDeleteDeal(dealID, nonce);
                    if (response.statusCode == 200) {
                      Navigator.pop(context);
                      Navigator.pop(context);
                      widget.dealDetailPageListener!(
                          index: widget.index, refresh: false);
                      _showToast(context,
                          UtilityMethods.getLocalizedString("deal_deleted"));
                    } else {
                      _showToast(
                        context,
