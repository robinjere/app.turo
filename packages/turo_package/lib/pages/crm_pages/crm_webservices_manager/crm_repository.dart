...
class CRMRepository extends PropertyRepository {
  ...
  ///
  /// CRM Activities
  Future<List> fetchActivitiesFromBoard(
  ...
  Future<List> fetchDealsAndLeadsFromActivity() async {
    ...
    if (response != null &&
        ...
        response.statusCode == 200) {
      final results = response.data;
      deals.add(_crmApiProvider
          .getCurrentParser()
          .parseDealsAndLeadsFromActivity(results));
    } else {
      if (response.statusCode == null) {
        deals = [response];
      }
  ...
  Future<List> fetchInquiryDetailNotesFromBoard(String enquiryId) async {
    ...
    final response = await _crmApiProvider
        .fetchInquiryDetailNotesFromBoardResponse(enquiryId);
  ...
  Future<Response> fetchAddCRMNotesResponse(
    ...
    final response =
        await _crmApiProvider.fetchAddCRMNotesResponse(dataMap, nonce);
    return response;
  }
  ...
  Future<List<dynamic>> fetchSavedSearch(int page, int perPage,
    ...
    savedSearchList = await super.fetchSavedSearches(page, perPage,
