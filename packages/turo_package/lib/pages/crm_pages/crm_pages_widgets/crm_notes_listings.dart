...
class CRMNotesListings extends StatefulWidget {
  final String id;
  final String fetch;
  const CRMNotesListings({Key? key, required this.id, required this.fetch})
      : super(key: key);
...
class _CRMNotesListingsState extends State<CRMNotesListings> {
  ...
  fetchNotes() {
    notesFromBoardList = [];
    _futureNotesFromBoard = fetchNotesFromBoard().then(
      (value) {
  ...
  Future<List<dynamic>> fetchNotesFromBoard() async {
    ...
    if (widget.fetch == FETCH_INQUIRY) {
      tempList =
          await _crmRepository.fetchInquiryDetailNotesFromBoard(widget.id);
                                ...
                                actions: <Widget>[
                                  ...
                                  TextButtonWidget(
                                    onPressed: () async {
                                      ...
                                      final response = await _crmRepository
                                          .fetchDeleteCRMNotesResponse(dataMap);
          ...
          child: ButtonWidget(
              ...
              onPressed: () async {
                if (formKey.currentState!.validate()) {
                  ...
                  final response = await _crmRepository
                      .fetchAddCRMNotesResponse(dataMap, nonce);
