...
import 'package:turo_package/pages/crm_pages/crm_inquiry/inquiry_detail_page.dart';
...
import 'package:turo_package/pages/realtor_information_page.dart';
...
class _CRMDetailsListingState extends State<CRMDetailsListing> {
  ...
  Future<List<dynamic>> fetchInquiryDetailMatchedFromBoard() async {
    ...
    if (widget.fetch == FETCH_INQUIRY_MATCHING) {
      tempList = await _crmRepository.fetchInquiryDetailMatchedFromBoard(
          widget.id, page);
    } else if (widget.fetch == FETCH_LEAD_INQUIRY) {
      tempList =
          await _crmRepository.fetchLeadsInquiriesFromBoard(widget.id, page);
    } else if (widget.fetch == FETCH_LEAD_VIEWED) {
      tempList =
          await _crmRepository.fetchLeadsViewedFromBoard(widget.id, page);
    } else if (widget.fetch == FETCH_LEAD_DETAIL) {
      tempList = await _crmRepository
          .fetchLeadsInquiriesFromBoard(widget.id, page, fetchLeadDetail: true);
    }
    ...
    return Scaffold(
      body: SafeArea(
        child: Container(
          ...
          child: widget.fetch == FETCH_INQUIRY_MATCHING
              ? Stack(
                  children: [
                    showListing(context, _futureCRMDetails),
                    if (_refreshController.isLoading)
                      const CRMPaginationLoadingWidget(),
                    if (showIndicatorWidget) loadingIndicatorWidget(),
      ...
      children: [
        ...
        Expanded(
          child: Checkbox(
            ...
            value: matchListingEmailIds.contains(item.id.toString()),
    ...
    return InkWell(
      ...
      child: Column(
        ...
        children: [
          ...
          CRMHeadingWidget(inquiry.propertyTypeName,
              secondHeading: inquiry.enquiryType),
          CRMContactDetail(
          ...
          CRMFeatures(inquiry.minBeds, inquiry.minBaths, inquiry.minArea,
              inquiry.minPrice),
  ...
  Widget imageWidget(
    ...
    String _imageUrl = "";
    if (infoDataMap["thumbnail"] != null &&
        infoDataMap["thumbnail"] is String) {
  ...
  Widget showLeadDetailWidget(CRMDealsAndLeads lead) {
    return Column(
      ...
      children: [
        ...
        const CRMDetailGap(),
        CRMIconBottomText(
            AppThemePreferences.messageIcon, "message", lead.message),
        const CRMDetailGap(),
        InkWell(
          onTap: () {
          ...
          child: CRMIconBottomText(AppThemePreferences.adsClickIcon, "source",
              lead.sourceLink!.isEmpty ? lead.source! : lead.sourceLink!),
        ),
        const CRMDetailGap(),
        CRMIconBottomText(
            AppThemePreferences.agentsIcon, "agent", lead.leadAgentName),
        const CRMDetailGap(),
      ],
    );
  }
  ...
  Widget errorWidget() {
    return Container(
      color: AppThemePreferences()
          .appTheme
          .shimmerEffectErrorWidgetBackgroundColor,
      child: Center(
  ...
  _showToast(BuildContext context, String msg) {
    ShowToastWidget(
        buildContext: context, text: msg, toastGravity: ToastGravity.CENTER);
  }
}
