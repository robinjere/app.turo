...
import 'package:turo_package/files/property_manager_files/property_manager.dart';
import 'package:turo_package/pages/add_property_v2/add_property_utilities.dart';
...
import 'package:turo_package/providers/api_providers/houzez_api_provider.dart';
import 'package:turo_package/providers/api_providers/property_api_provider.dart';
...
import 'package:turo_package/widgets/article_box_widgets/article_box_design_for_properties.dart';
import 'package:turo_package/widgets/article_box_widgets/article_box_design_for_uploads.dart';
import 'package:turo_package/widgets/custom_widgets/showModelBottomSheetWidget.dart';
import 'package:turo_package/widgets/custom_widgets/text_button_widget.dart';
import 'package:turo_package/widgets/data_loading_widget.dart';
import 'package:turo_package/widgets/dialog_box_widget.dart';
...
import 'package:turo_package/widgets/no_result_error_widget.dart';
import 'package:turo_package/widgets/private_note_widget.dart';
import 'package:turo_package/widgets/toast_widget.dart';
import 'package:pull_to_refresh/pull_to_refresh.dart';

import 'package:turo_package/widgets/article_box_widgets/article_box_design_for_drafts.dart';
import 'package:turo_package/widgets/generic_bottom_sheet_widget/generic_bottom_sheet_widget.dart';
...
class _PropertiesState extends State<Properties> with TickerProviderStateMixin {
  ...
  final PropertyApiProvider _propertyApiProvider =
      PropertyApiProvider(HOUZEZApiProvider());

  final RefreshController _refreshControllerForAllProperties =
      RefreshController(initialRefresh: false);
  final RefreshController _refreshControllerForMyProperties =
      RefreshController(initialRefresh: false);
  final RefreshController _refreshControllerForDraftProperties =
      RefreshController(initialRefresh: false);
      ...
      if (mounted) {
        setState(() {
          _uploadingPropertiesList =
              HiveStorageManager.readAddPropertiesDataMaps() ?? [];
  ...
  Future<List<dynamic>> fetchMyProperties(int page) async {
    ...
    List<dynamic> tempList =
        await _propertyBloc.fetchMyProperties(status, page, perPage, userId!);
    if (tempList == null ||
  ...
  Future<List<dynamic>> fetchAllProperties(int page) async {
    ...
    List<dynamic> tempList =
        await _propertyBloc.fetchAllProperties(status, page, perPage, null);
    if (tempList == null ||
        (tempList.isNotEmpty && tempList[0] == null) ||
        (tempList.isNotEmpty && tempList[0].runtimeType == Response)) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    if (userRole == ROLE_ADMINISTRATOR) {
      isAllPropertiesLoaded == false
          ? _tabsChildrenList.insert(0, noInternetWidget())
          : _tabsChildrenList.insert(
              0,
              showPropertiesList(_futureAllProperties!,
                  fromMyProperties: false));
    }
    ...
    return Scaffold(
      ...
      body: isInternetConnected == false
          ...
          : (userRole == ROLE_ADMINISTRATOR)
              ...
              : showPropertiesList(_futureMyProperties!,
                  fromMyProperties: true),
    );
  }
  ...
  PreferredSizeWidget appBarWithoutDraftsWidget() {
    List<Widget> _tabsList = [
      GenericTabWidget(
          label: UtilityMethods.getLocalizedString("all_properties")),
      GenericTabWidget(
          label: UtilityMethods.getLocalizedString("my_properties")),
    ];
    ...
    return AppBarWidget(
      ...
      bottom: userRole == ROLE_ADMINISTRATOR
          ? TabBar(
              controller: _tabController,
  ...
  PreferredSizeWidget appBarForDraftsWidget() {
    List<Widget> _tabsList = [
      GenericTabWidget(
          label: UtilityMethods.getLocalizedString("my_properties")),
      GenericTabWidget(
          label: UtilityMethods.getLocalizedString("draft_properties")),
    ];
    if (userRole == ROLE_ADMINISTRATOR) {
      _tabsList.insert(
          0,
  ...
  Widget showPropertiesList(Future<List<dynamic>> future,
  ...
  Widget genericOptionsOfBottomSheet(
    ...
    return SizedBox(
      ...
      child: Container(
        decoration:
            !showDivider ? null : AppThemePreferences.dividerDecoration(),
  ...
  Future<void>? changeStatusProperty(int? propertyId, int? propertyListIndex,
      Article? article, String? changeStatusValue) async {
    ...
    if (response.statusCode == 200) {
      Article item =
          _propertyApiProvider.getCurrentParser().parseArticle(response.data);
      setState(() {
        article = item;
        if (userRole == ROLE_ADMINISTRATOR) {
          if (_tabController!.index == 0) {
  ...
  Future<void> upgradeToFeatured(
      int? propertyId, int? propertyListIndex) async {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PaymentPage(
            productIds: Platform.isAndroid
                ? [MAKE_FEATURED_ANDROID_PRODUCT_ID]
                : [MAKE_FEATURED_IOS_PRODUCT_ID],
            propId: propertyId.toString(),
            isMembership: false,
            isFeaturedForPerListing:
  ...
  Future<void> payNow(int? propertyId, int? propertyListIndex) async {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => PaymentPage(
          productIds: Platform.isAndroid
              ? [PER_LISTING_ANDROID_PRODUCT_ID]
              : [PER_LISTING_IOS_PRODUCT_ID],
          propId: propertyId.toString(),
          isMembership: false,
        ),
      ),
  ...
  adminToggleFeatured(
      int? propertyId, int? propertyListIndex, bool setFeatured) async {
  ...
  setFeaturedStatus(
      ApiResponse response, int? propertyListIndex, bool isFeatured) {
    if (response.success) {
      if (userRole == ROLE_ADMINISTRATOR) {
        if (_tabController!.index == 0) {
  ...
  Future<void> setAsFeatured(int? propertyId, int? propertyListIndex) async {
    int featuredRemaining =
        int.tryParse(userPaymentStatusMap[featuredRemainingListingKey]) ?? 0;
    if (featuredRemaining > 0) {
      ...
      ApiResponse response =
          await _propertyBloc.fetchMakePropertyFeaturedResponse(dataMap);
      if (response.success) {
        Article? article;
        if (userRole == ROLE_ADMINISTRATOR) {
          if (_tabController!.index == 0) {
      ...
      ShowToastWidget(
          ...
          text: UtilityMethods.getLocalizedString(
              "You have used all the Featured listings in your package"));
    }
    Navigator.pop(context);
    getUserPaymentStatus();
  }

  Future<void> removeFromFeatured(
      int? propertyId, int? propertyListIndex) async {
    ApiResponse response = await _propertyBloc.fetchRemoveFromFeaturedResponse({
      "propid": propertyId,
    });
    if (response.success) {
      Article? article;
  ...
  setStatusSold(int? propertyId, int? propertyListIndex) async {
    ApiResponse response = await _propertyBloc.fetchSetSoldStatusResponse(
        {"listing_id": propertyId, "action": MARK_AS_SOLD});
    if (response.success) {
      if (userRole == ROLE_ADMINISTRATOR) {
        if (_tabController!.index == 0) {
          Article property = allPropertiesList[propertyListIndex!];
  ...
  setStatusExpire(int? propertyId, int? propertyListIndex) async {
    ApiResponse response = await _propertyBloc.fetchSetExpiredStatusResponse(
        {"listing_id": propertyId, "action": EXPIRED_LISTING});
    if (response.success) {
      if (userRole == ROLE_ADMINISTRATOR) {
        if (_tabController!.index == 0) {
          Article property = allPropertiesList[propertyListIndex!];
  ...
  setStatusPending(int? propertyId, int? propertyListIndex) async {
    ApiResponse response = await _propertyBloc.fetchSetPendingStatusResponse(
        {"listing_id": propertyId, "action": STATUS_ON_PENDING});
    if (response.success) {
      if (userRole == ROLE_ADMINISTRATOR) {
        if (_tabController!.index == 0) {
          Article property = allPropertiesList[propertyListIndex!];
  ...
  Widget noResultFoundPage({
    ...
    return NoResultErrorWidget(
      headerErrorText: headerText != null && headerText.isNotEmpty
          ? headerText
          : UtilityMethods.getLocalizedString("no_result_found"),
      bodyErrorText: bodyText != null && bodyText.isNotEmpty
          ? bodyText
          : UtilityMethods.getLocalizedString("no_properties_error_message"),
  ...
  Widget draftPropertiesWidget(List draftPropertiesList) {
    return SmartRefresher(
      ...
      child: draftPropertiesList == null || draftPropertiesList.isEmpty
          ? NoResultsFoundPage(
              hideGoBackButton: true,
              headerText: UtilityMethods.getLocalizedString(
                  "draft_properties_no_result_found_header_message"),
              bodyText: UtilityMethods.getLocalizedString(
                  "draft_properties_no_result_found_body_message"),
            )
              ...
              child: Column(
                children: draftPropertiesList.map((item) {
                  ...
                  return ArticleBoxDesignForDraftProperties(
                    article:
                        UtilityMethods.convertUploadAndDraftMapItemToArticle(
                            item),
                    heroID: "Hero $itemIndex",
                    onTap: () => _onEditDraftPressed(
                        itemIndex: itemIndex,
                        dataMap: UtilityMethods.convertMap(item)),
                    onActionButtonTap: () => onActionButtonPressed(
                        context: context, itemIndex: itemIndex, dataMap: item),
                  );
                }).toList(),
              ),
            ),
    ...
    return genericBottomSheetWidget(
      ...
      children: <Widget>[
        GenericBottomSheetOptionWidget(
          ...
          onPressed: () => _onEditDraftPressed(
              itemIndex: itemIndex, dataMap: dataMap, closeBottomSheet: true),
        ),
        GenericBottomSheetOptionWidget(
          ...
          style: AppThemePreferences()
              .appTheme
              .bottomSheetNegativeOptionsTextStyle!,
    ...
    return Column(
      children: list.map((propertyMap) {
        ...
        return UploadsArticleBox(
          article:
              UtilityMethods.convertUploadAndDraftMapItemToArticle(propertyMap),
          progress: progress,
...
class ShowPropertiesList extends StatelessWidget {
  ...
  void actionBottomSheet({
    ...
    if (TOUCH_BASE_PAYMENT_ENABLED_STATUS == perListing &&
    ...
    GenericOptionOfBottomSheet? featuredOption =
        getFeaturedOption(isFeatured, actionButtonMap, showPayNow);
    List<Widget> list = [
      ...
      if (userRole == ROLE_ADMINISTRATOR &&
          actionButtonMap["_status"] != "houzez_sold")
        GenericOptionOfBottomSheet(
          propertyId: actionButtonMap["_propertyId"],
      ...
      if (userRole == ROLE_ADMINISTRATOR &&
          actionButtonMap["_status"] != "expired")
        GenericOptionOfBottomSheet(
          propertyId: actionButtonMap["_propertyId"],
  ...
  GenericOptionOfBottomSheet? getFeaturedOption(
      bool isFeatured, Map<String, dynamic> actionButtonMap, bool showPayNow) {
    if (!isFeatured) {
      if (actionButtonMap["_author"] == userId) {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: InkWell(
        ...
        child: Container(
          decoration:
              AppThemePreferences.dividerDecoration(right: true, bottom: true),
...
class NoResultsFoundPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return NoResultErrorWidget(
      headerErrorText:
          headerText ?? UtilityMethods.getLocalizedString("no_result_found"),
      bodyErrorText: bodyText ??
          UtilityMethods.getLocalizedString("no_properties_error_message"),
