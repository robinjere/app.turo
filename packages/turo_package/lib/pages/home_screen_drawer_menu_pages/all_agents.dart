...
import 'package:turo_package/widgets/realtor_widgets/realtor_description_widget.dart';
import 'package:turo_package/widgets/realtor_widgets/realtor_image_widget.dart';
import 'package:turo_package/widgets/realtor_widgets/realtor_name_widget.dart';
import 'package:turo_package/widgets/realtor_widgets/realtor_position_widget.dart';
import 'package:turo_package/widgets/show_dialog_for_search.dart';
...
class _AllAgentsState extends State<AllAgents> {
  ...
  loadDataFromApi(bool forSearch, Map? agentSearchMap,
      {bool forPullToRefresh = true}) {
    if (forPullToRefresh) {
      if (isLoading) {
        return;
      }
  ...
  Future<List<dynamic>> fetchAgentsInfo(
      int page, bool forSearch, Map? agentSearchMap) async {
      ...
      body: Stack(
        children: [
          SmartRefresher(
            ...
            onLoading: () =>
                loadDataFromApi(false, null, forPullToRefresh: false),
            footer: CustomFooter(
          ...
          if (_refreshController.isLoading) paginationLoadingWidget(),
          if (showSearchDialog)
            ShowSearchDialog(
              fromAgent: true,
              searchDialogPageListener:
                  (bool showDialog, Map<String, dynamic>? agentSearchMap) {
  ...
  Widget buildAgentsList(
      BuildContext context, Future<List<dynamic>> futureAgentsList) {
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  String stringContent =
                      UtilityMethods.stripHtmlIfNeeded(item.content);
                              ...
                              Expanded(
                                ...
                                child: Container(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      RealtorNameWidget(
                                          title: item.title, tag: AGENTS_TAG),
                                      RealtorPositionWidget(
                                          position: "${item.agentPosition}"),
                                      RealtorNameWidget(
                                          title: "${item.agentCompany}",
                                          tag: AGENTS_TAG),
                                      RealtorDescriptionWidget(
