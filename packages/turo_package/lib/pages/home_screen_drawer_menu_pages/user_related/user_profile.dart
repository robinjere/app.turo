...
import 'package:turo_package/widgets/user_profile_related/user_avatar_widget.dart';
import 'package:turo_package/widgets/user_profile_related/user_name_widget.dart';
import 'package:turo_package/widgets/user_profile_related/user_related_actions_widget.dart';
...
class _UserProfileState extends State<UserProfile> {
  ...
  void initState() {
    ...
    paymentStatus =
        HiveStorageManager.readUserPaymentStatus()["enable_paid_submission"] ??
            "";

    generalNotifierLister = () {
      if (GeneralNotifier().change == GeneralNotifier.USER_PROFILE_UPDATE) {
        setState(() {
          userRole = UtilityMethods.chkRoleValueAndConvertToOption(
                  HiveStorageManager.getUserRole()) ??
              "";
          userEmail = HiveStorageManager.getUserEmail() ?? "";
          userName = HiveStorageManager.getUserName() ?? "";
          userAvatar = HiveStorageManager.getUserAvatar() ?? "";
      ...
      if (GeneralNotifier().change ==
          GeneralNotifier.USER_PAYMENT_STATUS_UPDATED) {
        if (mounted) {
          setState(() {
            paymentStatus = HiveStorageManager.readUserPaymentStatus()[
                    enablePaidSubmissionKey] ??
                "";
          });
        }
      }
