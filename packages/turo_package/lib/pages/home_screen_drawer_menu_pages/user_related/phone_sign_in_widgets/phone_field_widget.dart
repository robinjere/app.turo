...
class _PhoneNumberFieldWidgetState extends State<PhoneNumberFieldWidget> {
  ...
  void initState() {
    DefaultCountryCodeHook defaultCountryCodeHook =
        HooksConfigurations.defaultCountryCode;
    ...
    // Overwrite from Hook
    _textFormFieldCustomizationHook =
        HooksConfigurations.textFormFieldCustomizationHook;
    Map<String, dynamic>? tempMap = _textFormFieldCustomizationHook!();
    if (tempMap != null && tempMap.isNotEmpty) {
      if (tempMap.containsKey("labelTextStyle") &&
          tempMap["labelTextStyle"] != null) {
        labelTextStyle = tempMap["labelTextStyle"];
      }
      if (tempMap.containsKey("hintTextStyle") &&
          tempMap["hintTextStyle"] != null) {
        hintTextStyle = tempMap["hintTextStyle"];
      }
      if (tempMap.containsKey("additionalHintTextStyle") &&
          tempMap["additionalHintTextStyle"] != null) {
        additionalHintTextStyle = tempMap["additionalHintTextStyle"];
      }
      if (tempMap.containsKey("backgroundColor") &&
          tempMap["backgroundColor"] != null) {
        backgroundColor = tempMap["backgroundColor"];
      }
      if (tempMap.containsKey("focusedBorderColor") &&
          tempMap["focusedBorderColor"] != null) {
      ...
      if (tempMap.containsKey("borderRadius") &&
          tempMap["borderRadius"] != null) {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding:
          widget.ignorePadding ? const EdgeInsets.all(0.0) : widget.padding,
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: IntlPhoneField(
              initialCountryCode:
                  HooksConfigurations.defaultCountryCode() ?? 'TZ',
              decoration: InputDecoration(
                hintText: widget.hintText ??
                    UtilityMethods.getLocalizedString("phone"),
          ...
          if (widget.additionalHintText != null &&
              widget.additionalHintText!.isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(top: 5),
              child: AdditionalHintWidget(widget.additionalHintText!,
                  textStyle: additionalHintTextStyle),
            ),
        ],
      ),
    ...
    //   initialCountryCode: HooksConfigurations.defaultCountryCode() ?? 'TZ',
    //   decoration: InputDecoration(
