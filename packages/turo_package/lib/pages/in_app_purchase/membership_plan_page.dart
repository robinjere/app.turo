...
class _MembershipPlanPageState extends State<MembershipPlanPage> {
  ...
  MembershipPlanHook membershipPlanHook =
      HooksConfigurations.membershipPlanHook;
  MembershipPayWallDesignHook membershipPayWallDesignHook =
      HooksConfigurations.membershipPayWallDesignHook;
          ...
          if (articleSnapshot.hasData) {
            ...
            if (widget.fetchMembershipDetail) {
              ...
              list.removeWhere((element) =>
                  element.membershipPlanDetails!.packageVisible == "no");
  ...
  Widget noResultFoundPage() {
    return NoResultErrorWidget(
      ...
      bodyErrorText:
          UtilityMethods.getLocalizedString("oops_membership_not_exist"),
    );
  }
}
...
class UserMembershipPackageWidget extends StatefulWidget {
  ...
  const UserMembershipPackageWidget(this.userMembershipPackage, {Key? key})
      : super(key: key);

  @override
  State<UserMembershipPackageWidget> createState() =>
      _UserMembershipPackageWidgetState();
}

class _UserMembershipPackageWidgetState
    extends State<UserMembershipPackageWidget> {
  ...
  void _initializeListingData() {
    String remainingListingStr =
        widget.userMembershipPackage.remainingListings ?? "0.0";
    if (remainingListingStr == "-1") {
      isUnlimitedListing = true;
    } else {
      totalListing =
          double.tryParse(widget.userMembershipPackage.packListings ?? "0.0");
      remainingListing = double.tryParse(remainingListingStr) ?? 0.0;
      listingDataMap = {
        UtilityMethods.getLocalizedString("Listings Remaining"):
            remainingListing ?? 0.0,
      };
    }

    listingCenterText = (isUnlimitedListing
        ? UtilityMethods.getLocalizedString("Unlimited")
        : "${remainingListing!.round()}/${totalListing!.round()}")!;
  }

  void _initializeFeatureData() {
    String remainingFeatureStr =
        widget.userMembershipPackage.packFeaturedRemainingListings ?? "0.0";
    if (remainingFeatureStr == "-1") {
      isUnlimitedFeature = true;
    } else {
      totalFeature = double.tryParse(
          widget.userMembershipPackage.packFeaturedListings ?? "0.0");
      remainingFeature = double.tryParse(remainingFeatureStr) ?? 0.0;
      featureDataMap = {
        UtilityMethods.getLocalizedString("Featured Remaining"):
            remainingFeature ?? 0.0,
      };
    }
    featureCenterText = (isUnlimitedFeature
        ? UtilityMethods.getLocalizedString("Unlimited")
        : "${remainingFeature!.round()}/${totalFeature!.round()}")!;
  }

  Widget _buildCardWidget(Map<String, double> dataMap, String centerText,
      double? totalValue, String heading) {
    return Expanded(
      child: CardWidget(
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          GenericTextWidget("Your Current Package",
              style: const TextStyle(color: Colors.grey)),
          SizedBox(height: gap),
          GenericTextWidget(
            widget.userMembershipPackage.packTitle ?? "",
            style:
                AppThemePreferences().appTheme.membershipPackageNameTextStyle,
          ),
          SizedBox(height: gap),
          Row(
