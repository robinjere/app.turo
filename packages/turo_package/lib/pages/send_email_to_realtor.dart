...
class SendEmailToRealtorState extends State<SendEmailToRealtor>
  ...
  loadData() async {
    ...
    if (widget.informationMap.containsKey(SEND_EMAIL_SITE_NAME)) {
      siteName = widget.informationMap[SEND_EMAIL_SITE_NAME];
    }
    if (widget.informationMap.containsKey(SEND_EMAIL_THUMBNAIL)) {
      thumbnail = widget.informationMap[SEND_EMAIL_THUMBNAIL];
    }
    if (widget.informationMap.containsKey(SEND_EMAIL_LISTING_ID)) {
      listingId = widget.informationMap[SEND_EMAIL_LISTING_ID];
    }
    if (widget.informationMap.containsKey(SEND_EMAIL_APP_BAR_TITLE)) {
      appBarTitle = widget.informationMap[SEND_EMAIL_APP_BAR_TITLE];
    }
    if (widget.informationMap.containsKey(SEND_EMAIL_REALTOR_ID)) {
      agentAgencyId = widget.informationMap[SEND_EMAIL_REALTOR_ID];
    }
    if (widget.informationMap.containsKey(SEND_EMAIL_MESSAGE)) {
      messageContent = widget.informationMap[SEND_EMAIL_MESSAGE];
    }
    ...
    if (widget.informationMap.containsKey(SEND_EMAIL_REALTOR_TYPE)) {
      agentAgencyType = widget.informationMap[SEND_EMAIL_REALTOR_TYPE];
    }
    if (widget.informationMap.containsKey(SEND_EMAIL_REALTOR_EMAIL)) {
      agentAgencyEmail = widget.informationMap[SEND_EMAIL_REALTOR_EMAIL];
    }
  ...
  fetchNonce() async {
    ApiResponse response =
        await _propertyBloc.fetchContactPropertyAgentNonceResponse();
    if (response.success) {
        ...
        builder: (state) {
          return Column(
            ...
            children: <Widget>[
              ...
              state.errorText != null && isAgree == false
                  ? Padding(
  ...
  onSubmitPressed() {
    if (formKey.currentState!.validate()) {
      ...
      String listingId =
          widget.informationMap[SEND_EMAIL_LISTING_ID].toString();
      ...
      String key =
          widget.informationMap[SEND_EMAIL_LISTING_ID].toString() + email;
      ...
      if (infoMap != null && infoMap.isNotEmpty) {
        if (infoMap["email"] == email && infoMap["listing_id"] == listingId) {
          _showToast(context,
              UtilityMethods.getLocalizedString("message_already_sent"));
          if (mounted) {
            setState(() {
  ...
  sendEmail(Map<String, dynamic> currentInfoMap) async {
    ...
    final response = await _propertyBloc.fetchContactRealtorResponse(
        contactRealtorRequest, nonce);
