...
class ReportPropertyState extends State<ReportProperty> with ValidationMixin {
  ...
  fetchNonce() async {
    ApiResponse response =
        await _propertyBloc.fetchReportContentNonceResponse();
    if (response.success) {
  ...
  loadData() async {
    reportItemsList = [
      Term(
        id: 1,
        name: "Property already sold/rented",
        slug: "Property already sold/rented",
        parent: 0,
      ),
      Term(
        id: 2,
        name: "Incorrect price",
        slug: "Incorrect price",
        parent: 0,
      ),
      Term(
        id: 3,
        name: "Incorrect plot size",
        slug: "Incorrect plot size",
        parent: 0,
      ),
      Term(
        id: 4,
        name: "Incorrect location information",
        slug: "Incorrect location information",
        parent: 0,
      ),
      Term(
        id: 5,
        name: "Invalid/incorrect contact number",
        slug: "Invalid/incorrect contact number",
        parent: 0,
      ),
      Term(
        id: 6,
        name: "Property is for rent, not for sale",
        slug: "Property is for rent, not for sale",
        parent: 0,
      ),
      Term(
        id: 7,
        name: "The property shown here does not exist",
        slug: "The property shown here does not exist",
        parent: 0,
      ),
      Term(
        id: 8,
        name: "Incorrect number of bedrooms or rooms",
        slug: "Incorrect number of bedrooms or rooms",
        parent: 0,
      ),
      Term(
        id: 9,
        name: "Incorrect images",
        slug: "Incorrect images",
      ),
      Term(
        id: 10,
        name: "Switched off/Unreachable/ No answer",
        slug: "Switched off/Unreachable/ No answer",
        parent: 0,
      ),
      Term(
        id: 11,
        name: "Seller behaved improperly",
        slug: "Seller behaved improperly",
        parent: 0,
      ),
      Term(
        id: 12,
    ...
    message =
        "${UtilityMethods.getLocalizedString("Hello, The abuse report relates to")} $propertyName $propertyLink";
                  ...
                  Column(
                    ...
                    children: [
                      LabelWidget(UtilityMethods.getLocalizedString(
                          "Getting these information's will allow us to give you a feedback about this report.")),
                      const SizedBox(
                        height: 10,
                      ),
                      LabelWidget(
                          UtilityMethods.getLocalizedString("Type of abuse")),
              ...
              return MultiSelectDialogWidget(
                ...
                multiSelectDialogWidgetListener:
                    (List<dynamic> _selectedItemsList,
                        List<dynamic> _selectedReportSlugItemsList) {
  ...
  sendEmail() async {
    ...
    final response =
        await _propertyBloc.fetchReportContentResponse(dataMap, nonce);
    if (mounted) {
      setState(() {
    ...
    if (response == null) {
      ...
      if (tempString.contains("{")) {
        ...
        if (map['success']) {
          _showToast(
              context, UtilityMethods.getLocalizedString(map['message']));
          ...
          if (map["message"] != null) {
            ...
            _showToast(
                context, UtilityMethods.getLocalizedString(map['reason']));
          } else {
            _showToast(
