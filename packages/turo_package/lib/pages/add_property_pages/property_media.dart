    ...
    return Container(
      ...
      child: TextFormFieldWidget(
        ...
        hintText: UtilityMethods.getLocalizedString(
            "youtube_vimeo_swf_file_and_mov_file_are_supported"),
        additionalHintText: UtilityMethods.getLocalizedString("for_example") +
            PROPERTY_MEDIA_EXAMPLE_URL,
...
class _AddPropertyMediaWidgetState extends State<AddPropertyMediaWidget> {
    ...
    if (tempMap != null) {
      ...
      if (tempMap.containsKey(ADD_PROPERTY_FEATURED_IMAGE_LOCAL_ID)) {
        ...
        if (selectedImageId != null) {
          ...
          int index = _imageMapsList.indexWhere(
              (element) => element[PROPERTY_MEDIA_IMAGE_ID] == selectedImageId);
          selectedImageIndex = index;
        }
      }
      ...
      if (tempMap.containsKey(ADD_PROPERTY_FEATURED_IMAGE_LOCAL_INDEX)) {
        String? selectedImageId =
            tempMap[ADD_PROPERTY_FEATURED_IMAGE_LOCAL_INDEX];
        if (selectedImageId != null) {
          selectedImageIndex = int.parse(selectedImageId);
        }
      ...
      if (widget.isPropertyForUpdate) {
        List<dynamic> propertyAlreadyUploadedImages =
            tempMap[UPDATE_PROPERTY_IMAGES] ?? [];
        List<dynamic> propertyAlreadyUploadedImagesId =
            tempMap[ADD_PROPERTY_IMAGE_IDS] ?? [];
        List<dynamic> finalList = [];
  ...
  Future getImageMultiple() async {
    var pickedFiles = await picker.pickMultiImage(
        imageQuality: 90, maxHeight: 2000, maxWidth: 2000);
                  ...
                  child: item[PROPERTY_MEDIA_IMAGE_STATUS] == UPLOADED
                      ? Image.network(
                          ...
                          width: AppThemePreferences
                              .propertyMediaGridViewImageWidth,
                          height: AppThemePreferences
                              .propertyMediaGridViewImageHeight,
                        )
                      : Image.file(
                          File(item[PROPERTY_MEDIA_IMAGE_PATH]),
                          fit: BoxFit.cover,
                      ...
                      actions: <Widget>[
                        ...
                        TextButtonWidget(
                          onPressed: () async {
                            var _imageId = item[PROPERTY_MEDIA_IMAGE_ID];
                            String _imageStatus =
                                item[PROPERTY_MEDIA_IMAGE_STATUS];
                            if (_imageStatus == UPLOADED) {
                              Map<String, dynamic> deleteImageInfo = {
                                "thumb_id": _imageId,
                                "prop_id": propertyId,
                              };
                              final response = _propertyBloc
                                  .fetchDeleteImageFromEditProperty(
                                      deleteImageInfo, nonce);
                              if (response != null) {
                                updateImageList(context, item, index);
                              }
                            } else {
    ...
    if (mounted)
      setState(() {
        ...
        Map<String, dynamic> _imageMap =
            UtilityMethods.convertMap(_imageMapsList[index]);
        var status = _imageMap[PROPERTY_MEDIA_IMAGE_STATUS];
        if (status == UPLOADED) {
          var imageId = _imageMap[PROPERTY_MEDIA_IMAGE_ID];
