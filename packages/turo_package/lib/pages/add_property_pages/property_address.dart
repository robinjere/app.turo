...
import 'package:turo_package/models/address.dart';
import 'package:turo_package/models/address_search.dart';
...
import 'package:turo_package/widgets/shimmer_effect_error_widget.dart';
import 'package:uuid/uuid.dart';
import 'package:location/location.dart';

import 'package:turo_package/providers/api_providers/place_api_provider.dart';

typedef PropertyAddressPageListener = void Function(
...
class PropertyAddressPageState extends State<PropertyAddressPage> {
  ...
  Widget locationTextWidget() {
    return PropertyAddressHeaderWidget(
        text: UtilityMethods.getLocalizedString("location"));
  }
  ...
  Widget addPropertyAddress() {
    return Container(
      ...
      child: TextFormFieldWidget(
        ...
        hintText:
            UtilityMethods.getLocalizedString("enter_your_property_address"),
  ...
  Widget addPropertyCountry() {
    return !SHOW_COUNTRY_NAME_FIELD
        ? Container()
        : Container(
            padding: const EdgeInsets.only(top: 20.0),
            child: TextFormFieldWidget(
  ...
  Widget addPropertyStateOrCounty() {
    return !SHOW_STATE_COUNTY_FIELD
        ? Container()
        : Container(
            padding: const EdgeInsets.only(top: 20.0),
            child: TextFormFieldWidget(
  ...
  updateFormFields(LatLng? location) async {
    if (location != null) {
      ...
      var response =
          await PlaceApiProvider.getPlaceGeocodeDetailsFromCoordinates(cordLoc);
      Map? tempMap = response.data;

      if (tempMap != null &&
          tempMap.isNotEmpty &&
          tempMap.containsKey("results") &&
          tempMap["results"] != null) {
        ...
        if (addressDataList.isNotEmpty) {
          ...
          if (addressComponentsList != null &&
              addressComponentsList.isNotEmpty) {
      ...
      if (mounted) {
        setState(() {
          ...
          if (placeMap.containsKey(ADD_PROPERTY_STATE_OR_COUNTY) &&
            ...
            _addressStateTextController.text =
                placeMap[ADD_PROPERTY_STATE_OR_COUNTY];
          }
          ...
          if (placeMap.containsKey(ADD_PROPERTY_POSTAL_CODE) &&
            ...
            _addressZipOrPostalCodeTextController.text =
                placeMap[ADD_PROPERTY_POSTAL_CODE];
          }
...
class _LocationFromMapWidgetState extends State<LocationFromMapWidget> {
  ...
  final TextEditingController _addressLatitudeTextController =
      TextEditingController();
  final TextEditingController _addressLongitudeTextController =
      TextEditingController();
  ...
  void initState() {
    ...
    if (tempMap != null) {
      if (tempMap.containsKey(ADD_PROPERTY_LATITUDE) &&
          tempMap[ADD_PROPERTY_LATITUDE] != null) {
      ...
      if (tempMap.containsKey(ADD_PROPERTY_LONGITUDE) &&
          tempMap[ADD_PROPERTY_LONGITUDE] != null) {
  ...
  Widget googlePlaceSearchWidget() {
    return SHOW_SEARCH_BY_LOCATION
                    ...
                    if (mounted) {
                      setState(() {
                        ...
                        if (latitude != null && longitude != null) {
                          ...
                          if (currentPosition != null) {
                            currentPositionLink =
                                getStaticMapURLOfPosition(currentPosition!);
        ...
        : PropertyAddressHeaderWidget(
            text: UtilityMethods.getLocalizedString("map"));
  }
  ...
  Widget propertyAddressLoadingWidget() {
    return Container(
      color: AppThemePreferences().appTheme.shimmerEffectBaseColor,
      ...
      child: SizedBox(
        ...
        height: mapWidgetHeight,
        child: Align(
            alignment: Alignment.bottomCenter, child: BallBeatLoadingWidget()),
      ),
    );
  }
...
typedef AddressFromMapListener = void Function(
    LatLng locationLatLng, double zoomLevel);
            ...
            child: GestureDetector(
              onTap: () {
                if (_userSelectedPosition != null) {
                  widget.addressFromMapListener!(
                      _userSelectedPosition!, zoomLevel);
                } else {
              ...
              child: GenericTextWidget(
                ...
                style:
                    AppThemePreferences().appTheme.appBarActionWidgetsTextStyle,
              ),
            ),
          ),
        ],
