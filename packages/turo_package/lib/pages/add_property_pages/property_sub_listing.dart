...
import 'package:turo_package/widgets/add_property_widgets/multi_units_widgets/multi_units_listings_ids_widget.dart';
import 'package:turo_package/widgets/add_property_widgets/multi_units_widgets/multi_units_widget.dart';
...
class PropertySubListingPageState extends State<PropertySubListingPage> {
  ...
  void initState() {
    ...
    if (widget.propertyInfoMap != null && widget.propertyInfoMap!.isNotEmpty) {
      if (widget.propertyInfoMap!
              .containsKey(ADD_PROPERTY_FAVE_MULTI_UNITS_IDS) &&
          widget.propertyInfoMap![ADD_PROPERTY_FAVE_MULTI_UNITS_IDS] != null &&
          widget
              .propertyInfoMap![ADD_PROPERTY_FAVE_MULTI_UNITS_IDS].isNotEmpty) {
        _listingIDsTextController.text =
            widget.propertyInfoMap![ADD_PROPERTY_FAVE_MULTI_UNITS_IDS];
        selectedListingsIDs =
            widget.propertyInfoMap![ADD_PROPERTY_FAVE_MULTI_UNITS_IDS];
      }
      ...
      if (widget.propertyInfoMap!.containsKey(ADD_PROPERTY_FAVE_MULTI_UNITS) &&
        ...
        List<Map<String, dynamic>> list = List<Map<String, dynamic>>.from(
            widget.propertyInfoMap![ADD_PROPERTY_FAVE_MULTI_UNITS]);
        if (list.isNotEmpty) {
          ...
          child: Column(
            children: [
              ...
              if (userRole == ROLE_ADMINISTRATOR)
                MultiUnitsListingsIdsWidget(
                  selectedListingsIDs: selectedListingsIDs,
                  listener: (selectedIDs) {
                    selectedListingsIDs = selectedIDs;
                    widget.propertySubListingPageListener!(
                        _multiUnitsDataHolderList, selectedListingsIDs);
                  },
                ),
