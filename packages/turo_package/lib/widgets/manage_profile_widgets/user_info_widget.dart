          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                Divider(
                  thickness: 0,
                  color: AppThemePreferences.homeScreenDrawerTextColorDark,
                ),
                GenericTextWidget(
                  userEmail,
                  style: AppThemePreferences().appTheme.heading01TextStyle,
...
class UpdateProfileImageButtonWidget extends StatefulWidget {
  ...
  @override
  State<UpdateProfileImageButtonWidget> createState() =>
      _UpdateProfileImageButtonWidgetState();
}

class _UpdateProfileImageButtonWidgetState
    extends State<UpdateProfileImageButtonWidget> {
  ...
  fetchNonce() async {
    ApiResponse response =
        await _propertyBloc.fetchUpdateProfileImageNonceResponse();
    if (response.success) {
  ...
  Future<void> uploadPhoto(BuildContext context) async {
    ...
    final response = await _propertyBloc.fetchUpdateUserProfileImageResponse(
        userInfo, nonce);
    ...
    if (response == null || response.statusCode == null) {
      ...
      if (response.statusCode == 200) {
        ...
        ShowToastWidget(
          ...
          text:
              UtilityMethods.getLocalizedString("profile_updated_successfully"),
        );
      } else {
        widget.listener(showWaitingWidgetForPicUpload: false);
        ShowToastWidget(
