...
typedef GenericFormDropDownWidgetListener = void Function(
...
class GenericFormDropDownWidget extends StatefulWidget {
  ...
  @override
  State<GenericFormDropDownWidget> createState() =>
      _GenericFormDropDownWidgetState();
}
...
class _GenericFormDropDownWidgetState extends State<GenericFormDropDownWidget> {
                              ...
                              MaterialPageRoute(
                                builder: (context) => TermPickerFullScreen(
                                  ...
                                  termPickerFullScreenListener:
                                      (String pickedTerm, int? pickedTermId,
                                        ...
                                        if (pickedTerm.isNotEmpty) {
                                          _controller.text =
                                              UtilityMethods.getLocalizedString(
                                                  pickedTerm);
                                          updateValue(pickedTerm, pickedTermId,
                                              termType, pickedTermSlug);
                                        }
                                      });
                                    }
                                  },
  ...
  updateValue(
      dynamic value, int? termId, String? termType, String pickedTermSlug) {
    if (shouldHandleTermIdsData(termType)) {
  ...
  updateAddressRelatedFields() {
    if (infoDataMap != null && infoDataMap!.isNotEmpty) {
      if (apiKey == ADD_PROPERTY_STATE_OR_COUNTY) {
        ...
        } else if (shouldUpdateFieldData(
            ADD_PROPERTY_STATE_OR_COUNTY, _state)) {
          // update local variable of state
          _state = infoDataMap![ADD_PROPERTY_STATE_OR_COUNTY];
          _stateSlug = infoDataMap!["$ADD_PROPERTY_STATE_OR_COUNTY slug"];
