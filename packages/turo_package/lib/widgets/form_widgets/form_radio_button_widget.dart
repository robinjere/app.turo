...
typedef GenericFormRadioButtonWidgetListener = void Function(
...
class GenericFormRadioButtonWidget extends StatefulWidget {
  ...
  @override
  State<GenericFormRadioButtonWidget> createState() =>
      _GenericFormRadioButtonWidgetState();
}

class _GenericFormRadioButtonWidgetState
    extends State<GenericFormRadioButtonWidget> {
  String? apiKey;
  ...
  List<RadioItem> getOptionsList(FieldValues fieldValues) {
    ...
    if (fieldsMap != null && fieldsMap.isNotEmpty) {
      ...
      radioItemsList = fieldsMap.entries
          .map((mapItem) => RadioItem(label: mapItem.key, value: mapItem.value))
          .toList();
