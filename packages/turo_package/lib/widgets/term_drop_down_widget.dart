...
class _TermDropDownWidgetState extends State<TermDropDownWidget> {
  ...
  Widget build(BuildContext context) {
    if (widget.termDataList.isNotEmpty) {
      return DropdownButtonFormField(
        ...
        decoration:
            AppThemePreferences.formFieldDecoration(hintText: widget.hint),
        items: widget.termDataList.map((item) {
          return DropdownMenuItem(
            value: item.slug,
            child:
                GenericTextWidget(UtilityMethods.getLocalizedString(item.name)),
            onTap: () {
              dataItem = item;
            },
          );
        ...
        onChanged: (value) =>
            updatedValue(widget.comparisonOnTheBasisOf, dataItem),
      );
    }
  ...
  String? getSlug() {
    if (widget.comparisonOnTheBasisOf == TERM_NAME) {
      Term? term = getTermObjectWithName(
          dataList: widget.termDataList, name: widget.selectedDataItem);
      return term!.slug;
    } else if (widget.comparisonOnTheBasisOf == TERM_ID) {
      Term? term = getTermObjectWithId(
          dataList: widget.termDataList, id: widget.selectedDataItem);
      return term!.slug;
    }
