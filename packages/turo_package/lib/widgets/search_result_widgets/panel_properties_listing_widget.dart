...
import 'package:turo_package/widgets/generic_animate_icon_widget.dart';
...
import 'package:turo_package/widgets/search_result_widgets/panel_loading_widget.dart';
import 'package:turo_package/widgets/search_result_widgets/search_choice_chip_widget.dart';
import 'package:turo_package/widgets/search_result_widgets/sort_widget.dart';
...
class PanelPropertiesListingWidget extends StatefulWidget {
  ...
  @override
  State<PanelPropertiesListingWidget> createState() =>
      _PanelPropertiesListingWidgetState();
}

class _PanelPropertiesListingWidgetState
    extends State<PanelPropertiesListingWidget> {
  ...
  @override
  void initState() {
    _currentListingSortValue =
        sortByOptionsList.indexOf(DEFAULT_SORT_BY_OPTION);
    super.initState();
  }
              ...
              if (articleSnapshot.hasData) {
                ...
                if (isThereADifferenceInDataset(propertyList) || _sortFlag) {
                  ...
                  if (shouldPerformSortingOperations(propertyList)) {
                    propertyList =
                        performSortingOperationsAndAppendToOld(propertyList);
                  }
                                            ...
                                            children: [
                                              ...
                                              Padding(
                                                ...
                                                child: GenericTextWidget(
                                                  UtilityMethods
                                                      .getLocalizedString(
                                                          "showing_sorted_results",
                                                          inputWords: [
                                                        getTheCurrentSortOption()
                                                      ]),
                                                  style: AppThemePreferences()
                                                      .appTheme
  ...
  List<dynamic> performSortingOperationsAndAppendToOld(
  ...
  List<dynamic> performSortingOnNewListingsAndAppendToOld(
      List<dynamic> propertyList) {
    //propertyList.removeWhere((element) => element is AdWidget);
    ...
    // 2. SORT BY PRICE (LOW TO HIGH)
      ...
      propertyList.sort((a, b) {
        int aPrice = UtilityMethods.getCleanPriceForSorting(
            a.propertyInfo.price.isNotEmpty ? a.propertyInfo.price : "0");
        int bPrice = UtilityMethods.getCleanPriceForSorting(
            b.propertyInfo.price.isNotEmpty ? b.propertyInfo.price : "0");
    ...
    // 4. SORT BY AREA (LOW TO HIGH)
      ...
      propertyList.sort((a, b) {
        int aArea = UtilityMethods.getIntValueFromString(
            a.features.landArea.isNotEmpty ? a.features.landArea : "0");
        int bArea = UtilityMethods.getIntValueFromString(
            b.features.landArea.isNotEmpty ? b.features.landArea : "0");
  ...
  List<dynamic> sortByFirstOptions(List<dynamic> propertyList) {
    ...
    for (SortFirstByItem item in _sortFirstByItemsList) {
      if (item.defaultValue == 'on') {
        ...
        } else if (item.term != null && item.subTerm != null) {
          _sortFirstBySelectedOptions.add({
      ...
      for (int i = _sortFirstBySelectedOptions.length; i > 0; i--) {
        ...
        if (key == featuredSortItemKey) {
          _filteredList = propertyList
              .where((element) => element.propertyInfo!.isFeatured ?? false)
              .toList();
        }
        // SEPARATING LISTINGS ON THE BASIS OF PROPERTY TYPE
          ...
          _filteredList = propertyList
              .where((element) =>
                  element.features!.propertyTypeList.contains(value) ?? false)
              .toList();
        }
        // SEPARATING LISTINGS ON THE BASIS OF PROPERTY LABEL
          ...
          _filteredList = propertyList
              .where((element) =>
                  element.features!.propertyLabelList.contains(value) ?? false)
              .toList();
        }
        // SEPARATING LISTINGS ON THE BASIS OF PROPERTY STATUS
          ...
          _filteredList = propertyList
              .where((element) =>
                  element.features!.propertyStatusList.contains(value) ?? false)
              .toList();
        }
      ...
      body: refreshing == true
          ...
          : NoResultErrorWidget(
              ...
              bodyErrorText: UtilityMethods.getLocalizedString(
                  "no_properties_error_message_filter_page_search"),
              buttonText:
                  UtilityMethods.getLocalizedString("refine_search_text"),
