...
import 'package:turo_package/widgets/review_related_widgets/single_review_row.dart';
...
class _AllReviewsState extends State<AllReviews> {
  ...
  loadDataFromApi({bool forPullToRefresh = true}) {
    if (forPullToRefresh) {
      ...
      if (widget.fromProperty) {
        ...
        _futureFetchReviews =
            fetchReviews(widget.id, page, perPage, widget.reviewPostType);
      }
  ...
  Future<List<dynamic>> fetchPropertyReviews(
      int propertyId, int page, int perPage) async {
    if (page == 1) {
      setState(() {
        shouldLoadMore = true;
      });
    }
    List<dynamic> tempList = await _propertyBloc.fetchArticlesReviews(
        propertyId, page.toString(), perPage.toString());
  ...
  Future<List<dynamic>> fetchReviews(
      int propertyId, int page, int perPage, String type) async {
    if (page == 1) {
      setState(() {
        shouldLoadMore = true;
      });
    }
    List<dynamic> tempList = await _propertyBloc.fetchAgentAgencyAuthorReviews(
        propertyId, page.toString(), perPage.toString(), type);
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => RealtorInformationDisplayPage(
                  ...
                  agentType:
                      widget.reviewPostType == USER_ROLE_HOUZEZ_AGENT_VALUE
                          ? AGENT_INFO
                          : AGENCY_INFO,
                ),
              ),
  ...
  Widget articleReviews(
      BuildContext context, Future<List<dynamic>> futureFetchReviews) {
    return FutureBuilder<List<dynamic>>(
      future: futureFetchReviews,
      builder: (context, articleSnapshot) {
        isLoading = false;
  ...
  Widget noResultFoundPage() {
    return NoResultErrorWidget(
      ...
      bodyErrorText:
          UtilityMethods.getLocalizedString("oops_inquiries_not_exist"),
    );
  }
}
