...
class _CustomFieldsWidgetState extends State<CustomFieldsWidget> {
  ...
  void initState() {
    ...
    if (customFieldMap[CUSTOM_FIELDS_TYPE] == CUSTOM_FIELDS_TYPE_MULTI_SELECT) {
      ...
      multiSelectMap
          .forEach((k, v) => multiSelectList.add(CustomFieldModel(k, v)));
    }

    if (customFieldMap[CUSTOM_FIELDS_TYPE] ==
        CUSTOM_FIELDS_TYPE_CHECKBOX_LIST) {
    ...
    if (propertyInfoMap.isNotEmpty) {
      if (propertyInfoMap[customFieldMap[CUSTOM_FIELDS_ID]] != null &&
        ...
        if (customFieldMap[CUSTOM_FIELDS_TYPE] == CUSTOM_FIELDS_TYPE_TEXT ||
            customFieldMap[CUSTOM_FIELDS_TYPE] ==
                CUSTOM_FIELDS_TYPE_TEXT_AREA) {
          var value = propertyInfoMap[customFieldMap[CUSTOM_FIELDS_ID]];
          if (value is List) {
            textValue = value[0];
          } else {
      ...
      if (data.fieldId == customFieldMap[CUSTOM_FIELDS_ID]) {
        ...
        for (var data in tempList) {
          if (data == value) {
            ...
            for (int i = 0;
                i < customFieldMap[CUSTOM_FIELDS_VALUES].length;
                i++) {
              if (customFieldMap[CUSTOM_FIELDS_VALUES][i] ==
                  selectedRadioButtonStr) {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: widget.formItemIndex == 0
          ? null
  ...
  Widget getGenericCustomFieldsWidget() {
    ...
    if (customFieldMap[CUSTOM_FIELDS_TYPE] == CUSTOM_FIELDS_TYPE_TEXT) {
      return textWidget(
        labelTextStyle:
            widget.fromFilterPage ? filterPageHeadingTitleTextStyle : null,
      );
    ...
    } else if (customFieldMap[CUSTOM_FIELDS_TYPE] ==
        CUSTOM_FIELDS_TYPE_SELECT) {
              ...
              return userChoiceFromRadioButton(
                ...
                onChanged: (value) {
                  setState(() {
                    ...
                    customFieldDataPropertyInfoMap["selectedRadioButton"] =
                        value;
                    customFieldDataPropertyInfoMap[
                            customFieldMap[CUSTOM_FIELDS_ID]] =
                        customFieldMap[CUSTOM_FIELDS_VALUES][index];
                  });
                  widget
                      .customFieldsPageListener(customFieldDataPropertyInfoMap);
                },
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField(
          ...
          onChanged: (value) {
            if (value != null) {
              customFieldDataPropertyInfoMap[customFieldMap[CUSTOM_FIELDS_ID]] =
                  value;
  ...
  Widget multiSelectDropDownViewWidget(List dataList,
...
//           dataItemsList: multiSelectMap,
          ...
          multiSelectDialogWidgetListener: (List<dynamic> selectedItemsList,
              List<dynamic> listOfSelectedItemsSlugs) {
            Map map = {};
            if (widget.fromFilterPage) {
              Map tempMap = {
                for (var item in listOfSelectedItemsSlugs) '$item': '$item'
              };
              map.addAll(tempMap);
            } else {
              Map tempMap = {
                for (var item in selectedItemsList) '$item': '$item'
              };
              map.addAll(tempMap);
            }
            customFieldDataPropertyInfoMap[customFieldMap[CUSTOM_FIELDS_ID]] =
