    ...
    return Container(
      ...
      child: GenericTextWidget(
        UtilityMethods.getLocalizedString(
            "what_information_do_you_want_to_display_in_agent_data_container"),
        textAlign:
            UtilityMethods.isRTL(context) ? TextAlign.right : TextAlign.left,
...
class ContactInformationBodyWidget extends StatefulWidget {
  ...
  @override
  State<ContactInformationBodyWidget> createState() =>
      _ContactInformationBodyWidgetState();
}

class _ContactInformationBodyWidgetState
    extends State<ContactInformationBodyWidget> {
  ...
  void initState() {
    ...
    if (_userRole == USER_ROLE_HOUZEZ_AGENCY_VALUE) {
      ...
      if (realtorIdStr.isNotEmpty &&
          realtorIdStr.containsKey(FAVE_AUTHOR_AGENCY_ID)) {
  ...
  assignValues() {
    ...
    if (tempMap != null) {
      ...
      if (useAuthorInfo && realtorId != null) {
        ...
        dataMap[ADD_PROPERTY_FAVE_AGENCY] = [
          realtorIdStr[FAVE_AUTHOR_AGENCY_ID]
        ];
      ...
      if (mounted) {
        ...
        do {
          if (_userRole == USER_ROLE_HOUZEZ_AGENCY_VALUE) {
            if (useAuthorInfo) {
              tempList =
                  await _propertyBloc.fetchAgencyAllAgentList(realtorId!);
            } else {
              tempList =
                  await _propertyBloc.fetchAgencyAgentInfoList(realtorId!);
            }
          } else {
            tempList =
                await _propertyBloc.fetchAllAgentsInfoList(page, perPage);
          }
        ...
        do {
          if (_userRole == USER_ROLE_HOUZEZ_AGENCY_VALUE) {
            ...
            if (agencyUserId != null) {
              tempList =
                  await _propertyBloc.fetchSingleAgencyInfoList(agencyUserId);
            }
          } else {
            tempList =
                await _propertyBloc.fetchAllAgenciesInfoList(page, perPage);
          }
