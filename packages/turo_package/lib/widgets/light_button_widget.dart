...
class LightButtonWidget extends StatelessWidget {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: Row(
          ...
          children: [
            iconOnRightSide ? Container() : showIconWithMargin(icon),
            centeredContent
                ? content(icon, text, fontSize!, iconOnRightSide, textColor)
                : Expanded(
                    child: content(
                        icon, text, fontSize!, iconOnRightSide, textColor)),
  ...
  Widget showIconWithMargin(Icon? icon) {
    if (icon == null) {
      return Container();
    }
    return Container(
        margin: const EdgeInsets.only(left: 5), child: showIcon(icon));
  }
  ...
  Widget content(Icon? icon, String text, double fontSize, bool rightIcon,
      Color? textColor) {
    return Padding(
      padding: icon == null
      ...
      child: GenericTextWidget(
        ...
        style: TextStyle(
          color:
              textColor ?? AppThemePreferences().appTheme.selectedItemTextColor,
          fontSize: fontSize,
        ),
      ),
    );
