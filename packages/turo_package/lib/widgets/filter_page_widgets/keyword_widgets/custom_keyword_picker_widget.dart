...
import 'package:turo_package/widgets/filter_page_widgets/keyword_widgets/keyword_input_field_widget.dart';
  ...
  void initState() {
    ...
    if (widget.dataMap != null &&
      ...
      _keywordFiltersDataMap =
          Map<String, dynamic>.from(widget.dataMap![keywordFiltersKey]);
    }
    ...
    if (_uniqueKey != null && _uniqueKey!.isNotEmpty) {
      ...
      if (_uniqueKey != null &&
          ...
          _keywordFiltersDataMap[_uniqueKey!][keywordFiltersValueKey]
              is String) {
        _selectedOptionsString =
            _keywordFiltersDataMap[_uniqueKey!][keywordFiltersValueKey];
        textEditingController.text =
            UtilityMethods.getLocalizedString(_selectedOptionsString ?? "");
      }
    }

    if (widget.pickerType == stringPickerKey ||
        widget.pickerType == dropDownPicker) {
      if (_optionsString!.isNotEmpty) {
        _optionsList = UtilityMethods.getListFromString(_optionsString);
      }

      if (_selectedOptionsString != null &&
          _selectedOptionsString!.isNotEmpty) {
        _selectedOptionsList =
            UtilityMethods.getListFromString(_selectedOptionsString);
      }
    }
            ...
            return StringPicker(
                ...
                stringPickerListener: (List<dynamic> _selectedItemsList) {
                  if (_selectedItemsList.isNotEmpty) {
                    _selectedOptionsList =
                        List<String>.from(_selectedItemsList);
                  } else {
                    _selectedOptionsList = [];
                  }

                  widget.listener(
                      UtilityMethods.getStringFromList(_selectedOptionsList));
                });
          }
          return Container();
        }
