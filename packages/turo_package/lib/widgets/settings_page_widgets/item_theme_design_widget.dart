...
class _ItemThemeDesignWidgetState extends State<ItemThemeDesignWidget> {
  ...
  Widget build(BuildContext context) {
    return !SHOW_THEME_RELATED_SETTINGS
        ? Container()
        : GenericSettingsWidget(
            ...
            headingSubTitleText: UtilityMethods.getLocalizedString(
                "customise_your_item_theme_design"),
  ...
  onItemDesignChange(String? val) {
    ...
    ShowToastWidget(
      ...
      text: UtilityMethods.getLocalizedString(
          "item_design_is_successfully_updated"),
    );
  }
  ...
  onExploreTermDesignChanged(String? val) {
    ...
    widget.itemDesignNotifier
        .setExploreByTypeItemDesign(_selectedExploreByTypeDesign!);
    ShowToastWidget(
      buildContext: context,
      text: UtilityMethods.getLocalizedString(
          "explore_by_type_is_successfully_updated"),
    );
  }
}
...
class ThemeDesignDropDownWidget extends StatefulWidget {
  ...
  @override
  State<ThemeDesignDropDownWidget> createState() =>
      _ThemeDesignDropDownWidgetState();
}
...
class _ThemeDesignDropDownWidgetState extends State<ThemeDesignDropDownWidget> {
  ...
  void initState() {
    ...
    propertyMetaDataListForThemeDesign = [
      _propertyMetaData01,
      _propertyMetaData02,
      _propertyMetaData03
    ];
              ...
              return DropdownMenuItem<String>(
                ...
                child: ExpandablePanel(
                  theme: ExpandableThemeData(
                      iconColor: AppThemePreferences.appIconsMasterColorLight),
                  header: GenericTextWidget(item),
                  expanded: SizedBox(
                    height: getExpandableOptionHeight(
                        tag: widget.tag, design: item),
                    child: getExpandableOptionDesignWidget(
                        tag: widget.tag, design: item),
                  ),
                  collapsed: Container(),
                ),
  ...
  double? getExpandableOptionHeight(
      {required String tag, required String design}) {
