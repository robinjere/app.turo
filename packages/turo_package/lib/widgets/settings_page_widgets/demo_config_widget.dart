  ...
  Widget build(BuildContext context) {
    return !SHOW_DEMO_CONFIGURATIONS
        ...
        : GenericSettingsWidget(
            headingText:
                UtilityMethods.getLocalizedString("demo_configuration"),
            headingSubTitleText:
                UtilityMethods.getLocalizedString("enter_your_own_wordpress"),
            body: Padding(
              padding: const EdgeInsets.only(top: 10.0),
              child: TextFormFieldWidget(
                additionalHintText:
                    UtilityMethods.getLocalizedString("web_example"),
                ignorePadding: true,
                readOnly: true,
                hintText: UtilityMethods.getLocalizedString(
                    "enter_your_wordpress_url"),
                suffixIcon: Icon(
    ...
    if (formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (communicationProtocol == HTTP) {
          uri = Uri.http(
              _wordpressUrlAuthority, HOUZEZ_META_DATA_PATH, queryParameters);
        ...
        if (response.statusCode == 200) {
          ...
          HiveStorageManager.deleteAllData();
          HiveStorageManager.storeUrl(url: _wordpressUrl);
          HiveStorageManager.storeUrlAuthority(
              authority: _wordpressUrlAuthority);
          HiveStorageManager.storeCommunicationProtocol(
              protocol: communicationProtocol);
          UtilityMethods.navigateToRouteByPushAndRemoveUntil(
              context: context, builder: (context) => const MyHomePage());
      ...
      actions: <Widget>[
        TextButtonWidget(
          ...
          child: GenericTextWidget(
              UtilityMethods.getLocalizedString("plug_in_text")),
        ),
        Padding(
