...
class PropertyManager with ChangeNotifier {
    ...
    final subscription = uploader.updates.listen((update) {
      switch (update) {
        ...
        case TaskStatusUpdate():
          ...
          List<dynamic> _propertyDataMapsList =
          ...
          if (_propertyDataMapsList.isNotEmpty) {
            if (update.responseStatusCode == 200 &&
              ...
              Map<String, dynamic> _propertyDataMap =
                  UtilityMethods.convertMap(_propertyDataMapsList[0]);

              ///
              /// if property uploading is already in Progress, then there is no need to do anything
              if (_propertyDataMap[ADD_PROPERTY_UPLOAD_STATUS] ==
                  ADD_PROPERTY_UPLOAD_STATUS_IN_PROGRESS) {
              ...
              if (_propertyDataMap[ADD_PROPERTY_FLOOR_PLANS] != null &&
                  _propertyDataMap[ADD_PROPERTY_FLOOR_PLANS].isNotEmpty) {
                // List<Map<String, dynamic>> floorPlansList = _propertyDataMap[ADD_PROPERTY_FLOOR_PLANS];
                List<dynamic> floorPlansList =
                    _propertyDataMap[ADD_PROPERTY_FLOOR_PLANS] ?? [];
                // print("Floor Plans List Before: $floorPlansList");
                for (int i = 0; i < floorPlansList.length; i++) {
                  Map<String, dynamic> tempFloorPlanMap =
                      UtilityMethods.convertMap(floorPlansList[i]);
                  if (tempFloorPlanMap.containsKey(favePlanPendingImage) &&
                    ...
                    Map<String, dynamic> _imageMap = UtilityMethods.convertMap(
                        floorPlansList[i][favePlanPendingImage]);
                ...
                for (var item in floorPlansList) {
                  Map<String, dynamic> tempFloorPlansMap =
                      UtilityMethods.convertMap(item);
                  if (tempFloorPlansMap.containsKey(favePlanPendingImage) &&
                    ...
                    if (tempFloorPlansMap[favePlanPendingImage]
                                [PROPERTY_MEDIA_IMAGE_STATUS] !=
                            null &&
                        tempFloorPlansMap[favePlanPendingImage]
                                [PROPERTY_MEDIA_IMAGE_STATUS] ==
                            PENDING) {
                      totalFloorPlansPendingImages++;
                    }
              ...
              List<dynamic> _imageList =
                  _propertyDataMap[ADD_PROPERTY_PENDING_IMAGES_LIST] ?? [];
              for (int i = 0; i < _imageList.length; i++) {
                Map<String, dynamic> _imageMap =
                    UtilityMethods.convertMap(_imageList[i]);
                String? imageTaskId = _imageMap[IMAGE_TASK_ID];
                if (imageTaskId != null && taskId == imageTaskId) {
              ...
              for (int j = 0; j < _imageList.length; j++) {
                Map<String, dynamic> _imageMap =
                    UtilityMethods.convertMap(_imageList[j]);
              ...
              if (totalPendingImages == 0 &&
                  totalFloorPlansPendingImages == 0) {
                _totalProgress = 80;
                notifyListeners();
                ...
                String? _featuredImageLocalId =
                    _propertyDataMap[ADD_PROPERTY_FEATURED_IMAGE_LOCAL_ID];
                ...
                for (int j = 0; j < _imageList.length; j++) {
                  ...
                  ///
                  if (_featuredImageLocalId != null &&
                      attachmentId != null &&
                      _imageMap[PROPERTY_MEDIA_IMAGE_ID] != null &&
                      _featuredImageLocalId ==
                          _imageMap[PROPERTY_MEDIA_IMAGE_ID]) {
                    _propertyDataMap[ADD_PROPERTY_FEATURED_IMAGE_ID] =
                        attachmentId;
                ...
                if (_propertyDataMap[ADD_PROPERTY_UPLOAD_STATUS] ==
                  ...
                  _propertyDataMap[ADD_PROPERTY_UPLOAD_STATUS] =
                      ADD_PROPERTY_UPLOAD_STATUS_IN_PROGRESS;
  ...
  uploadProperty() async {
    ...
    if (_propertyDataMap.isNotEmpty) {
      ...
      ApiResponse? nonceResponse =
          await _propertyBloc.fetchAddImageNonceResponse();
          ...
          for (int i = 0; i < floorPlansList.length; i++) {
            ...
            if (imageStatus != null && imageStatus == PENDING) {
              ...
              final String imageName =
                  _imageMap[PROPERTY_MEDIA_IMAGE_NAME] ?? "";
              ...
              if (imagePath != null) {
                final task = UploadTask(
                  ...
                  headers: {
                    "Authorization":
                        "Bearer ${HiveStorageManager.getUserToken()}"
                  },
                  fields: {
                    "user_id": '${HiveStorageManager.getUserId()}',
                    kAddImageNonceVariable: imageNonce!
                  },
                  fileField: "property_upload_file",
                  httpRequestMethod: 'POST',
                  updates: Updates.statusAndProgress,
                  displayName:
                      '$imageUploadingLabel $uploadingFloorPlansPendingImagesCount/$totalFloorPlansPendingImages',
                );
          ...
          if (imageStatus != null && imageStatus == PENDING) {
            ...
            if (imagePath != null) {
              final task = UploadTask(
                ...
                filename: imageName,
                headers: {
                  "Authorization": "Bearer ${HiveStorageManager.getUserToken()}"
                },
                fields: {
                  "user_id": '${HiveStorageManager.getUserId()}',
                  kAddImageNonceVariable: imageNonce!
                ...
                updates: Updates.statusAndProgress,
                displayName:
                    '$imageUploadingLabel $uploadingImageCount/$totalPendingImages',
              );
  ...
  Future<void> uploadPropertyViaDio(
      Map<String, dynamic> _propertyDataMap) async {
    /// Clear All previous Tasks from Uploader Queue
    // print("Clearing All previous Tasks from Uploader Queue...");
    // uploader.clearUploads();
    uploader.taskQueues.clear();
    ...
    var isPropertyForUpdate =
        (_propertyDataMap[ADD_PROPERTY_ACTION] == ADD_PROPERTY_ACTION_UPDATE);
    ...
    if (isPropertyForUpdate) {
      ...
      response = await _propertyBloc.fetchUpdatePropertyResponse(
          _propertyDataMap, nonce!);
    } else {
      // print('Uploading Property...');
      response = await _propertyBloc.fetchAddPropertyResponse(
          _propertyDataMap, nonce!);
    }
