...
import 'package:turo_package/files/generic_methods/general_notifier.dart';
...
import 'package:turo_package/files/icons/cupertino_icons.dart';
import 'package:turo_package/files/icons/material_icons.dart';
import 'package:turo_package/models/blog_models/blog_details_page_layout.dart';
import 'package:turo_package/models/sort_first_by_item.dart';
import 'package:turo_package/models/form_related/houzi_form_page.dart';
import 'package:turo_package/models/navbar_item.dart';
import 'package:turo_package/pages/add_property_v2/add_property_v2.dart';
import 'package:turo_package/pages/in_app_purchase/membership_plan_page.dart';
import 'package:turo_package/pages/in_app_purchase/payment_page.dart';
import 'package:turo_package/pages/quick_add_property/quick_add_property.dart';
import 'package:turo_package/pages/quick_add_property/quick_add_property_v2.dart';
import 'package:turo_package/providers/state_providers/user_log_provider.dart';
import 'package:turo_package/l10n/string_extension.dart';
import 'package:turo_package/models/article.dart';
import 'package:turo_package/models/drawer.dart';
import 'package:turo_package/models/home_config.dart';
import 'package:turo_package/models/property_detail_page_config.dart';
import 'package:turo_package/models/property_meta_data.dart';
import 'package:turo_package/pages/app_settings_pages/about.dart';
import 'package:turo_package/pages/app_settings_pages/dark_mode_setting.dart';
import 'package:turo_package/pages/app_settings_pages/language_settings.dart';
import 'package:turo_package/pages/filter_page.dart';
import 'package:turo_package/pages/home_screen_drawer_menu_pages/user_related/user_signin.dart';
import 'package:turo_package/pages/property_details_page.dart';
import 'package:turo_package/pages/search_result.dart';
...
import 'package:turo_package/pages/app_settings_pages/web_page.dart';

class UtilityMethods {
  // Platform messages are asynchronous, so we initialize in an async method.
  static Future<void> initTrackingPermissionAndInitializeMobileAds(
      AppTrackingPermissionCallback trackingCallback) async {
  ...
  static String makePriceCompact(String inputPrice, String pricePrefix,
      {bool priceOnly = false}) {
    CompactPriceFormatterHook compactPriceFormatterHook =
        HooksConfigurations.compactPriceFormatter;
  ...
  static String priceFormatter(
      String propertyPrice, String firstPrice, String pricePrefix) {
  ...
  static int getCleanPriceForSorting(String propertyPrice) {
    int price =
        int.tryParse(propertyPrice.replaceAll(RegExp(r'[^\d]+'), '')) ?? 0;
    return price;
  }
  ...
  static NumberFormat getNumberFormat() {
    ...
    if (THOUSAND_SEPARATOR == ",") {
      numberFormat =
          NumberFormat("#$THOUSAND_SEPARATOR###$DECIMAL_POINT_SEPARATOR##");
      ...
      numberFormat = NumberFormat.currency(
          locale: 'fr', customPattern: '#,###', decimalDigits: 0);
    }
    return numberFormat!;
  }

  static void navigateToRoute(
      {required context, required WidgetBuilder builder}) {
  ...
  static void navigateToSearchResultScreen({
    ...
    final NavigateToSearchResultScreenListener?
  ...
  static List getSubTypeItemRelatedList(
      String subtype, List subTypeValuesList) {
    ...
    if (dataList.isNotEmpty) {
      for (var item in subTypeValuesList) {
        if (item != allString && item != userSelectedString) {
          var tempItem =
              dataList.firstWhereOrNull((element) => element.slug == item);
          if (tempItem != null) {
  ...
  static String getPlacesApiLockedCountriesFormattedString(
  ...
  static userLogOut({required context, required WidgetBuilder builder}) {
  ...
  static saveBottomNavigationBarConfigFile(Map appConfigurationsData) {
    List<dynamic> bottomNavBarConfig =
        appConfigurationsData[bottomNavBarLayoutConfiguration] ?? [];
  ...
  static saveDrawerConfigFile(Map appConfigurationsData) {
    final drawerConfig =
        drawerLayoutConfigFromJson(json.encode(appConfigurationsData));
  ...
  static List<dynamic> readDrawerConfigFile() {
    ...
    String drawerConfigData =
        HiveStorageManager.readDrawerConfigListData() ?? "";
  ...
  static saveFilterPageConfigFile(Map appConfigurationsData) {
    ...
    List<dynamic> filterPageConfigElementsList =
        appConfigurationsData[searchPageLayoutConfiguration];
    HiveStorageManager.storeFilterConfigListData(
        json.encode(filterPageConfigElementsList));
  }

  static savePropertyDetailPageConfigFile(Map appConfigurationsData) {
    final propertyDetailPageConfig =
        propertyDetailPageLayoutFromJson(json.encode(appConfigurationsData));
    List<dynamic> propertyDetailPageConfigList =
        propertyDetailPageConfig.propertyDetailPageLayout!;
    HiveStorageManager.storePropertyDetailConfigListData(
        json.encode(propertyDetailPageConfigList));
  }

  static saveAddPropertyConfigFile(Map appConfigurationsData) {
    List<dynamic> addPropertyConfigList =
        appConfigurationsData[addPropertyLayoutApiConfiguration] ?? [];
    if (addPropertyConfigList.isNotEmpty) {
      HiveStorageManager.storeAddPropertyConfigurations(
          json.encode(addPropertyConfigList));
    }
  }

  static saveQuickAddPropertyConfigFile(Map appConfigurationsData) {
    List<dynamic> configList =
        appConfigurationsData[quickAddPropertyLayoutApiConfiguration] ?? [];
    if (configList.isNotEmpty) {
      HiveStorageManager.storeQuickAddPropertyConfigurations(
          json.encode(configList));
    }
  }

  static saveBlogDetailPageConfigFile(Map appConfigurationsData) {
    BlogDetailsPageLayout blogDetailPageLayout =
        BlogDetailsPageLayout.fromRawJson(json.encode(appConfigurationsData));
    List<BlogDetailPageLayout> configList =
        blogDetailPageLayout.blogDetailPageLayout ?? [];
    if (configList.isNotEmpty) {
  ...
  static List<HouziFormPage> readAddPropertyConfigFile() {
    ...
    String addPropertyConfigData =
        HiveStorageManager.readAddPropertyConfigurations() ?? "";
  ...
  static saveSortFirstByConfigFile(Map appConfigurationsData) {
    List<dynamic> _sortFirstByConfigList =
        appConfigurationsData[SORT_FIRST_BY_CONFIG] ?? [];
    if (_sortFirstByConfigList.isNotEmpty) {
      HiveStorageManager.storeSortFirstByConfigData(
          json.encode(_sortFirstByConfigList));
    }
  }
  ...
  static updateTouchBaseDataAndConfigurations(
    ...
    if (ENABLE_API_CONFIG) {
      ...
      if (newConfigMap.isNotEmpty) {
        ...
        if (oldConfigJson == null || oldConfigJson.isEmpty) {
          ...
          Map<String, dynamic> oldConfigMap =
              convertMap(jsonDecode(oldConfigJson));
        ...
        if (SHOW_ADS) {
          UtilityMethods.initTrackingPermissionAndInitializeMobileAds(
              ({status}) {});
        }
      }
    }
  ...
  static updateAppConfigurationsInStorage(Map<String, dynamic> newConfigMap) {
    ...
    if (newConfigMap.containsKey(defaultHomeApiConfiguration) &&
      ...
      final DefaultHomePageHook defaultHomePageHook =
          HooksConfigurations.defaultHomePage;
  ...
  static String getPropertyMetaDataItemNameWithSlug({
    ...
    if (dataList.isNotEmpty) {
      Term? item = dataList.firstWhereOrNull(
          (element) => (element is Term && element.slug == slug));
  ...
  static Term? getPropertyMetaDataObjectWithId({
    ...
    if (dataList != null && dataList.isNotEmpty) {
      Term? item = dataList
          .firstWhereOrNull((element) => (element is Term && element.id == id));
  ...
  static Term? getPropertyMetaDataObjectWithItemName({
    ...
    if (dataList != null && dataList.isNotEmpty) {
      Term? item = dataList.firstWhereOrNull(
          (element) => (element is Term && element.name == name));
  ...
  static Map<String, dynamic> getParentAndChildCategorizedMap(
      {required List<dynamic> metaDataList}) {
  ...
  static MaterialPageRoute AboutPageRoute() {
    ...
    SHOW_DEMO_CONFIGURATIONS
        ? pageRoute = MaterialPageRoute(builder: (context) => About())
        : pageRoute = MaterialPageRoute(
            builder: (context) => WebPage(
                COMPANY_URL, UtilityMethods.getLocalizedString("about")));
    return pageRoute;
  }
  ...
  static MaterialPageRoute PrivacyPolicyPageRoute() {
    MaterialPageRoute pageRoute = MaterialPageRoute(
        builder: (context) => WebPage(APP_PRIVACY_URL,
  ...
  static MaterialPageRoute TermsAndConditionsPageRoute() {
    MaterialPageRoute pageRoute = MaterialPageRoute(
        builder: (context) => WebPage(APP_TERMS_URL,
            UtilityMethods.getLocalizedString("terms_and_conditions")));
    return pageRoute;
  }
  ...
  static Future<Map<String, dynamic>> getMapForNearByProperties() async {
    Position position =
        await getCurrentPosition(locationAccuracy: LocationAccuracy.high);
    Map<String, dynamic> dataMap = {
  ...
  static Widget navigateToAddPropertyPage({bool navigateToQuickAdd = false}) {
    ...
    if (userPaymentStatusMap.isNotEmpty) {
      ...
      if (TOUCH_BASE_PAYMENT_ENABLED_STATUS != freeListing ||
        ...
        /// set property featured. There is 3 different plans of membership
          ...
          bool hasMembership =
              userPaymentStatusMap[userHasMembershipKey] ?? false;
          if (remainingListing != -1 &&
              (!hasMembership || remainingListing <= 0)) {
    ...
    if (navigateToQuickAdd) {
      String addPropertyConfigData =
          HiveStorageManager.readQuickAddPropertyConfigurations() ?? "";
  ...
  static bool showUpgradePlanPage() {
    ...
    if (userPaymentStatusMap.isNotEmpty) {
      if (TOUCH_BASE_PAYMENT_ENABLED_STATUS != freeListing ||
        ...
        if (userPaymentStatusMap[enablePaidSubmissionKey] == freePaidListing) {
          return false;
        } else if (userPaymentStatusMap[enablePaidSubmissionKey] ==
            perListing) {
          if (remainingListing <= 0) {
            return true;
          }
  ...
  static Map? getMapItemValueFromMap(
      {required Map inputMap, required String key}) {
    if (inputMap.containsKey(key) &&
        inputMap[key] != null &&
        inputMap[key] is Map &&
  ...
  static dynamic getItemValueFromMap(
      {required Map inputMap, required String key}) {
  ...
  static String getTimeAgoFormat(
      {required String? time, String? locale = 'en'}) {
  ...
  static String getFormattedDate(String? gmtDate) {
    ...
    if (gmtDate != null && gmtDate.isNotEmpty) {
      ...
      sanitizedDate =
          DateFormat.yMMMMd(HiveStorageManager.readLanguageSelection() ?? 'en')
              .format(dateTime);
    }
  ...
  static Map<String, dynamic> removeElementFromLists(
  ...
  static int getIntValueFromString(String inputString) {
    int _integerValue =
        int.tryParse(inputString.replaceAll(RegExp(r'[^\d]+'), '')) ?? 0;
  ...
  static String getHomeLocationString(
    Map map, {
