...
class HouzezParser implements ApiParserInterface {
  ...
  ApiResponse<String> parseNormalApiResponse(Response response) {
    if (response.statusCode == HttpStatus.ok) {
      ...
      if ((response.statusCode == HttpStatus.forbidden || response.statusCode == HttpStatus.badRequest) && response.data is Map) {
        Map? map = response.data;
  ...
  CheckNotifications parseCheckNotificationsResponse(Map<String, dynamic> json) {
    return CheckNotifications(
      ...
      lastCheckedNotificationString: json["last_checked_notification"] is String ? json["last_checked_notification"] : null,
      lastCheckedNotificationDateTime: (json["last_checked_notification"] == null || json["last_checked_notification"] is! String) ? null : DateTime.parse(json["last_checked_notification"]),
    );
  }
}
