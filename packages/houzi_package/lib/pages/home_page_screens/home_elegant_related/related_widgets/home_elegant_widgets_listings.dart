...
class _HomeElegantListingsWidgetState extends State<HomeElegantListingsWidget> {
  ...
  void initState() {
    ...
    generalNotifierLister = () {
      if (GeneralNotifier().change == GeneralNotifier.CITY_DATA_UPDATE) {
        ...
        else if(homeConfigMap[sectionTypeKey] == propertyKey && listDependsOnCitySelection()){
          setState(() {
            Map<String, dynamic> map = HiveStorageManager.readSelectedCityInfo() ?? {};
            String cityId = UtilityMethods.valueForKeyOrEmpty(map, CITY_ID);
            if(homeConfigMap[subTypeValueCityKey] != cityId) {
    ...
    try {
      ...
      /// Fetch Properties
        ...
        if (listDependsOnCitySelection()) {
          ...
          String city = UtilityMethods.valueForKeyOrEmpty(map, CITY);
          homeConfigMap[titleKey] = titleForSectionBasedOnCitySelection(city);
          if (map.isNotEmpty && city.isNotEmpty && city.toLowerCase() != 'all') {
  ...
  bool listDependsOnCitySelection() {
    ...
    return (homeConfigMap[subTypeKey] == propertyCityDataType && homeConfigMap[subTypeValueKey] == userSelectedString)
        || (subTypeList.contains(propertyCityDataType) && subTypeValueList.contains(userSelectedString));
  }
